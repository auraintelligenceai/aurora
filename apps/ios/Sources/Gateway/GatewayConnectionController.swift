...
final class GatewayConnectionController {
    ...
    private func makeConnectOptions() -> GatewayConnectOptions {
        ...
        return GatewayConnectOptions(
            ...
            permissions: [:],
            clientId: "aura_intelligence-ios",
            clientMode: "node",
    ...
    private func currentCaps() -> [String] {
        var caps = [aura_intelligenceCapability.canvas.rawValue, aura_intelligenceCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(aura_intelligenceCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(aura_intelligenceCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = aura_intelligenceLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(aura_intelligenceCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            aura_intelligenceSystemCommand.notify.rawValue,
            aura_intelligenceSystemCommand.which.rawValue,
            aura_intelligenceSystemCommand.run.rawValue,
            aura_intelligenceSystemCommand.execApprovalsGet.rawValue,
            aura_intelligenceSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(aura_intelligenceCapability.camera.rawValue) {
            commands.append(aura_intelligenceCameraCommand.list.rawValue)
            commands.append(aura_intelligenceCameraCommand.snap.rawValue)
            commands.append(aura_intelligenceCameraCommand.clip.rawValue)
        }
        if caps.contains(aura_intelligenceCapability.location.rawValue) {
            commands.append(aura_intelligenceLocationCommand.get.rawValue)
        }
