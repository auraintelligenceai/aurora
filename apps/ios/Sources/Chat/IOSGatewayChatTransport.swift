...
struct IOSGatewayChatTransport: aura_intelligenceChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> aura_intelligenceChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(aura_intelligenceChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> aura_intelligenceChatHistoryPayload {
        ...
        return try JSONDecoder().decode(aura_intelligenceChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [aura_intelligenceChatAttachmentPayload]) async throws -> aura_intelligenceChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [aura_intelligenceChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(aura_intelligenceChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(aura_intelligenceGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: aura_intelligenceGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: aura_intelligenceChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: aura_intelligenceAgentEventPayload.self)
                        {
