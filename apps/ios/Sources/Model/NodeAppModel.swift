...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = aura_intelligenceCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = aura_intelligenceCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = aura_intelligenceCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = aura_intelligenceCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = aura_intelligenceCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__aura_intelligence__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: aura_intelligenceLocationMode) async -> Bool {
                        ...
                        onInvoke: { [weak self] req in
                            guard let self else {
                                return BridgeInvokeResponse(
                                    id: req.id,
                                    ok: false,
                                    error: aura_intelligenceNodeError(
                                        code: .unavailable,
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for aura_intelligence:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case aura_intelligenceLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case aura_intelligenceCanvasCommand.present.rawValue,
                 aura_intelligenceCanvasCommand.hide.rawValue,
                 aura_intelligenceCanvasCommand.navigate.rawValue,
                 aura_intelligenceCanvasCommand.evalJS.rawValue,
                 aura_intelligenceCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case aura_intelligenceCanvasA2UICommand.reset.rawValue,
                 aura_intelligenceCanvasA2UICommand.push.rawValue,
                 aura_intelligenceCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case aura_intelligenceCameraCommand.list.rawValue,
                 aura_intelligenceCameraCommand.snap.rawValue,
                 aura_intelligenceCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case aura_intelligenceScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: aura_intelligenceNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: aura_intelligenceNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(aura_intelligenceLocationGetParams.self, from: req.paramsJSON)) ??
            aura_intelligenceLocationGetParams()
        ...
        let payload = aura_intelligenceLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case aura_intelligenceCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(aura_intelligenceCanvasPresentParams.self, from: req.paramsJSON)) ??
                aura_intelligenceCanvasPresentParams()
        ...
        case aura_intelligenceCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case aura_intelligenceCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(aura_intelligenceCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case aura_intelligenceCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(aura_intelligenceCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case aura_intelligenceCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(aura_intelligenceCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case aura_intelligenceCanvasA2UICommand.reset.rawValue:
            ...
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case aura_intelligenceCanvasA2UICommand.push.rawValue, aura_intelligenceCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == aura_intelligenceCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(aura_intelligenceCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try aura_intelligenceCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(aura_intelligenceCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try aura_intelligenceCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.clawdbotA2UI) return JSON.stringify({ ok: false, error: "missing aura_intelligenceA2UI" });
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case aura_intelligenceCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case aura_intelligenceCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(aura_intelligenceCameraSnapParams.self, from: req.paramsJSON)) ??
                aura_intelligenceCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case aura_intelligenceCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(aura_intelligenceCameraClipParams.self, from: req.paramsJSON)) ??
                aura_intelligenceCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(aura_intelligenceScreenRecordParams.self, from: req.paramsJSON)) ??
            aura_intelligenceScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> aura_intelligenceLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return aura_intelligenceLocationMode(rawValue: raw) ?? .off
    }
