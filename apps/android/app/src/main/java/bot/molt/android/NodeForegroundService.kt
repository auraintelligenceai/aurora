...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "aura_intelligence Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "aura_intelligence Node · Connected" else "aura_intelligence Node"
          val voiceSuffix =
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "aura_intelligence node connection status"
        setShowBadge(false)
      }
