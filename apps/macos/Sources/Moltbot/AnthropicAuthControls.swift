...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: aura_intelligenceOAuthStore.AnthropicOAuthStatus = aura_intelligenceOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: aura_intelligenceOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = aura_intelligenceOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = aura_intelligenceOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try aura_intelligenceOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. aura_intelligence can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: aura_intelligenceOAuthStore.AnthropicOAuthStatus,
