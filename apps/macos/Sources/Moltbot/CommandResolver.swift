...
enum CommandResolver {
    private static let projectRootDefaultsKey = "aura_intelligence.gatewayProjectRootPath"
    private static let helperName = "aura_intelligence"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/aura_intelligence.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/aura_intelligence")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let aura_intelligencePaths = self.aura_intelligenceManagedPaths(home: home)
        if !aura_intelligencePaths.isEmpty {
            extras.insert(contentsOf: aura_intelligencePaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + aura_intelligencePaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func aura_intelligenceManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".aura")
    ...
    static func aura_intelligenceExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectaura_intelligenceExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/aura_intelligence.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyaura_intelligenceInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.aura_intelligenceExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func aura_intelligenceNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let aura_intelligencePath = self.projectaura_intelligenceExecutable(projectRoot: root) {
                return [aura_intelligencePath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "aura_intelligence", subcommand] + extraArgs
            }
            if let aura_intelligencePath = self.aura_intelligenceExecutable(searchPaths: searchPaths) {
                return [aura_intelligencePath, subcommand] + extraArgs
            }

            let missingEntry = """
            aura_intelligence entrypoint missing (looked for dist/index.js or bin/aura_intelligence.js); run pnpm build.
            """
    ...
    // Existing callers still refer to aura_intelligenceCommand; keep it as node alias.
    static func aura_intelligenceCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
        searchPaths: [String]? = nil) -> [String]
    {
        self.aura_intelligenceNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real aura_intelligence CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/aura_intelligence"
        ...
        if command -v aura_intelligence >/dev/null 2>&1; then
          CLI="$(command -v aura_intelligence)"
          aura_intelligence \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/aura_intelligence.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/aura_intelligence.js"
            node "$PRJ/bin/aura_intelligence.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent aura_intelligence"
          pnpm --silent aura_intelligence \(quotedArgs);
        else
          echo "aura_intelligence CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? aura_intelligenceConfigFile.loadDict()
