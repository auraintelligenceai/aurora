...
actor PortGuardian {
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "aura_intelligence", "tsx", "pnpm", "bun"]
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            // The gateway daemon may listen as `aura_intelligence` or as its runtime (`node`, `bun`, etc).
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a aura_intelligence listener as expected.
            if cmd.contains("aura_intelligence"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
