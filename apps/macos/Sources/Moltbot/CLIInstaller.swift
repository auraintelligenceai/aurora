...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("aura_intelligence").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing aura_intelligence CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed aura_intelligence \($0)." } ?? "Installed aura_intelligence."
