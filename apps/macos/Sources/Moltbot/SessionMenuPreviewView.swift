...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "aura", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> aura_intelligenceSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: aura_intelligenceSessionPreviewEntry,
    ...
    private static func cache(payload: aura_intelligenceSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: aura_intelligenceChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [aura_intelligenceKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [aura_intelligenceKit.AnyCodable]) -> [aura_intelligenceChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(aura_intelligenceChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: aura_intelligenceChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: aura_intelligenceChatMessage) -> Bool {
    ...
    private static func toolNames(for message: aura_intelligenceChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: aura_intelligenceChatMessage) -> String? {
