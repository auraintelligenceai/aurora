...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so aura_intelligence can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = aura_intelligenceLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = aura_intelligenceLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(aura_intelligenceLocationMode.off.rawValue)
                Text("While Using").tag(aura_intelligenceLocationMode.whileUsing.rawValue)
                Text("Always").tag(aura_intelligenceLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = aura_intelligenceLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: aura_intelligenceLocationMode {
        aura_intelligenceLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: aura_intelligenceLocationMode) async -> Bool {
