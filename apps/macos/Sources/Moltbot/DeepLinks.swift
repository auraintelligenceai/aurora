...
private let deepLinkLogger = Logger(subsystem: "aura", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "aura_intelligence is paused", message: "Unpause aura_intelligence to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run aura_intelligence agent?", message: body) else { return }
        }
