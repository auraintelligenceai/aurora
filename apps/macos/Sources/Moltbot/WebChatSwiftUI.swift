...
private let webChatSwiftLogger = Logger(subsystem: "aura", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: aura_intelligenceChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> aura_intelligenceChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(aura_intelligenceChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> aura_intelligenceChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                aura_intelligenceGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    aura_intelligenceChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    aura_intelligenceAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<aura_intelligenceChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any aura_intelligenceChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = aura_intelligenceChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: aura_intelligenceChatView(
            viewModel: vm,
        ...
        contentViewController: NSViewController) -> NSWindow
    {
        ...
        case .window:
            let window = NSWindow(
                ...
                backing: .buffered,
                defer: false)
            window.title = "aura_intelligence Chat"
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<aura_intelligenceChatView>) -> NSViewController
    {
