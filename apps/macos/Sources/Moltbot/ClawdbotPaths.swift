...
enum aura_intelligencePaths {
    ...
    static var configURL: URL {
        if let override = aura_intelligenceEnv.path(self.configPathEnv) {
            return URL(fileURLWithPath: override)
        }
        return self.stateDirURL.appendingPathComponent("aura_intelligence.json")
    }
