...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "aura", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous aura_intelligence run already has an SSH listener on the expected port (common after restarts),
