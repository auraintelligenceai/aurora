...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: aura_intelligenceProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: aura_intelligenceKit.AnyCodable]()) {
                $0[$1.key] = aura_intelligenceKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: aura_intelligenceProtocol.AnyCodable?) -> [String: aura_intelligenceProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: aura_intelligenceProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: aura_intelligenceKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: aura_intelligenceProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("aura_intelligence.control.heartbeat")
    static let controlAgentEvent = Notification.Name("aura_intelligence.control.agent")
}
