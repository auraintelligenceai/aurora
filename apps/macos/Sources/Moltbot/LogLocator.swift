...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/aura_intelligence")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("aura_intelligence-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("aura_intelligence-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/aura_intelligence/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("aura_intelligence") && $0.pathExtension == "log" }
            .max { lhs, rhs in
