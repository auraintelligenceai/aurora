...
enum aura_intelligenceLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = aura_intelligenceOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = aura_intelligenceFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        aura_intelligenceLogging.bootstrapIfNeeded()
        let label = aura_intelligenceLogging.makeLabel(subsystem: subsystem, category: category)
...
struct aura_intelligenceOSLogHandler: LogHandler {
...
struct aura_intelligenceFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = aura_intelligenceLogging.parseLabel(self.label)
