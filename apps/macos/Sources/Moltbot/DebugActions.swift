...
enum DebugActions {
    private static let verboseDefaultsKey = "aura_intelligence.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "aura_intelligence.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "aura_intelligence", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".aura/aura_intelligence.json")
        guard
