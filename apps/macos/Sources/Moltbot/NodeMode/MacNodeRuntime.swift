...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case aura_intelligenceCameraCommand.snap.rawValue,
                 ...
                 aura_intelligenceCameraCommand.list.rawValue:
                return try await self.handleCameraInvoke(req)
            case aura_intelligenceLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MacNodeScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            case aura_intelligenceSystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case aura_intelligenceSystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case aura_intelligenceSystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case aura_intelligenceSystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case aura_intelligenceSystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case aura_intelligenceCanvasA2UICommand.push.rawValue,
             aura_intelligenceCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleA2UIPush(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        let messages: [aura_intelligenceKit.AnyCodable]
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__aura_intelligence__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(aura_intelligenceSystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(aura_intelligenceSystemWhichParams.self, from: req.paramsJSON)
    ...
    private func resolveSystemRunApproval(
        ...
        params: aura_intelligenceSystemRunParams,
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(aura_intelligenceSystemNotifyParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> aura_intelligenceLocationMode {
        ...
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: aura_intelligenceNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: aura_intelligenceCanvasSnapshotFormat,
        maxWidth: Int?,
