...
actor GatewayWizardClient {
    ...
    private func sendConnect() async throws {
        ...
        let clientId = "aura_intelligence-macos"
        let clientMode = "ui"
        let role = "operator"
        let scopes: [String] = []
        let client: [String: ProtoAnyCodable] = [
            "id": ProtoAnyCodable(clientId),
            "displayName": ProtoAnyCodable(Host.current().localizedName ?? "aura_intelligence macOS Wizard CLI"),
