...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/aura_intelligence/canvas/<session>/`).
...
public let controlSocketPath = FileManager()
    ...
    .appendingPathComponent("Library/Application Support/aura_intelligence/control.sock")
    .path
