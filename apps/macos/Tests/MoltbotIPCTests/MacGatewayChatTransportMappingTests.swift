...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: aura_intelligenceProtocol.AnyCodable(["ok": aura_intelligenceProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: aura_intelligenceProtocol.AnyCodable(["ok": aura_intelligenceProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = aura_intelligenceProtocol.AnyCodable([
            "runId": aura_intelligenceProtocol.AnyCodable("run-1"),
            "sessionKey": aura_intelligenceProtocol.AnyCodable("main"),
            "state": aura_intelligenceProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: aura_intelligenceProtocol.AnyCodable(["a": aura_intelligenceProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
