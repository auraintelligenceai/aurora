...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: aura_intelligenceProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: aura_intelligenceProtocol.AnyCodable] = [
            "presence": .init([aura_intelligenceProtocol.AnyCodable(entry)]),
        ]
        let payload = aura_intelligenceProtocol.AnyCodable(payloadMap)
