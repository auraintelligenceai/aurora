...
struct ChatMessageBubble: View {
    let message: aura_intelligenceChatMessage
    let style: aura_intelligenceChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? aura_intelligenceChatTheme.userText : aura_intelligenceChatTheme.assistantText
    ...
    private var inlineAttachments: [aura_intelligenceChatMessageContent] {
    ...
    private var toolCalls: [aura_intelligenceChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [aura_intelligenceChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? aura_intelligenceChatTheme.userBubble
        }
        if self.style == .onboarding {
            return aura_intelligenceChatTheme.onboardingAssistantBubble
        }
        return aura_intelligenceChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return aura_intelligenceChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: aura_intelligenceChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? aura_intelligenceChatTheme.userText : aura_intelligenceChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: aura_intelligenceChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(aura_intelligenceChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(aura_intelligenceChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [aura_intelligenceChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: aura_intelligenceChatTheme.assistantText)
            }
        }
    }
}
