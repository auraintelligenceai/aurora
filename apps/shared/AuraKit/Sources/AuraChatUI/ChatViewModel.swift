...
private let chatUILogger = Logger(subsystem: "bot.molt", category: "aura_intelligenceChatUI")

@MainActor
@Observable
public final class aura_intelligenceChatViewModel {
    public private(set) var messages: [aura_intelligenceChatMessage] = []
    ...
    public var attachments: [aura_intelligencePendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [aura_intelligenceChatPendingToolCall] = []
    public private(set) var sessions: [aura_intelligenceChatSessionEntry] = []
    private let transport: any aura_intelligenceChatTransport
    ...
    private var pendingToolCallsById: [String: aura_intelligenceChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any aura_intelligenceChatTransport) {
    ...
    public var sessionChoices: [aura_intelligenceChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [aura_intelligenceChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [aura_intelligenceChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: aura_intelligencePendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [aura_intelligenceChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: aura_intelligenceChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [aura_intelligenceChatMessage]) -> [aura_intelligenceChatMessage] {
        var result: [aura_intelligenceChatMessage] = []
    ...
    private static func dedupeKey(for message: aura_intelligenceChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [aura_intelligenceChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> aura_intelligenceChatAttachmentPayload in
            aura_intelligenceChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> aura_intelligenceChatSessionEntry {
        aura_intelligenceChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: aura_intelligenceChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: aura_intelligenceChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: aura_intelligenceAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = aura_intelligenceChatPendingToolCall(
    ...
    private func addImageAttachment(url: URL?, data: Data, fileName: String, mimeType: String) async {
        ...
        self.attachments.append(
            aura_intelligencePendingAttachment(
                url: url,
                data: data,
                fileName: fileName,
                mimeType: mimeType,
    ...
    private static func previewImage(data: Data) -> aura_intelligencePlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
