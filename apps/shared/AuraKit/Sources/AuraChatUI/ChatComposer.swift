...
@MainActor
struct aura_intelligenceChatComposer: View {
    @Bindable var viewModel: aura_intelligenceChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(aura_intelligenceChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(aura_intelligenceChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \aura_intelligencePendingAttachment.id)
                { (att: aura_intelligencePendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            aura_intelligencePlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(aura_intelligenceChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(aura_intelligenceChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(aura_intelligenceChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(aura_intelligenceChatTheme.subtleCard)
        .clipShape(Capsule())
    }
