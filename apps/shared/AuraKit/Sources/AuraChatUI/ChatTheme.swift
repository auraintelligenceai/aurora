...
enum aura_intelligenceChatTheme {
    #if os(macOS)
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("aura_intelligenceChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("aura_intelligenceChatTheme.onboardingAssistantBubble"),
...
enum aura_intelligencePlatformImageFactory {
    static func image(_ image: aura_intelligencePlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
