...
import Foundation
import Testing
@testable import aura_intelligenceChatUI
...
private final class TestChatTransport: @unchecked Sendable, aura_intelligenceChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [aura_intelligenceChatHistoryPayload]
    private let sessionsResponses: [aura_intelligenceChatSessionsListResponse]

    private let stream: AsyncStream<aura_intelligenceChatTransportEvent>
    private let continuation: AsyncStream<aura_intelligenceChatTransportEvent>.Continuation

    init(
        historyResponses: [aura_intelligenceChatHistoryPayload],
        sessionsResponses: [aura_intelligenceChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<aura_intelligenceChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> aura_intelligenceChatHistoryPayload {
        ...
        return self.historyResponses.last ?? aura_intelligenceChatHistoryPayload(
        ...
        attachments _: [aura_intelligenceChatAttachmentPayload]) async throws -> aura_intelligenceChatSendResponse
    {
        ...
        return aura_intelligenceChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> aura_intelligenceChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? aura_intelligenceChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: aura_intelligenceChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = aura_intelligenceChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = aura_intelligenceChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { aura_intelligenceChatViewModel(sessionKey: "main", transport: transport) }
        ...
        transport.emit(
            .agent(
                aura_intelligenceAgentEventPayload(
                    runId: sessionId,
                    seq: 1,
                    stream: "assistant",
        ...
        transport.emit(
            .chat(
                aura_intelligenceChatEventPayload(
                    runId: runId,
                    sessionKey: "main",
                    state: "final",
                    message: nil,
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = aura_intelligenceChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = aura_intelligenceChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = aura_intelligenceChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { aura_intelligenceChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
