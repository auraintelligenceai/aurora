...
Write-Host ""
Write-Host "  aura_intelligence Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:aura_intelligence_INSTALL_METHOD)) {
        $InstallMethod = $env:aura_intelligence_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:aura_intelligence_GIT_DIR)) {
        $GitDir = $env:aura_intelligence_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:aura_intelligence_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:aura_intelligence_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:aura_intelligence_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "aura_intelligence")
}
...
# Check for existing aura_intelligence installation
function Check-Existingaura_intelligence {
    try {
        $null = Get-Command aura_intelligence -ErrorAction Stop
    Write-Host "[*] Existing aura_intelligence installation detected" -ForegroundColor Yellow
    return $true
    } catch {
        return $false
    }
...
function Ensure-aura_intelligenceOnPath {
    if (Get-Command aura_intelligence -ErrorAction SilentlyContinue) {
        return $true
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
        ...
        if (Test-Path (Join-Path $npmBin "aura_intelligence.cmd")) {
            return $true
        }
    }

    Write-Host "[!] aura_intelligence is not on PATH yet." -ForegroundColor Yellow
...
# Install aura_intelligence
function Install-aura_intelligence {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    # Use aura_intelligence package for beta, aura_intelligence for stable
    $packageName = "aura_intelligence"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "aura_intelligence"
    }
    Write-Host "[*] Installing aura_intelligence ($packageName@$Tag)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host "  iwr -useb https://aura_intelligence.ai/install.ps1 | iex" -ForegroundColor Cyan
            }
    ...
    Write-Host "[OK] aura_intelligence installed" -ForegroundColor Green
}

# Install aura_intelligence from GitHub
function Install-aura_intelligenceFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $repoUrl = "https://github.com/auraintelligenceai/aurora.git"
    Write-Host "[*] Installing aura_intelligence from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    $cmdPath = Join-Path $binDir "aura_intelligence.cmd"
    ...
    Write-Host "[OK] aura_intelligence wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        aura_intelligence doctor --non-interactive
    } catch {
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:aura_intelligence_GIT_DIR)) {
        return $env:aura_intelligence_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "aura_intelligence")
}
...
function Main {
    ...
    $isUpgrade = Check-Existingaura_intelligence
    ...
    # Step 2: aura_intelligence
    if ($InstallMethod -eq "git") {
        $finalGitDir = $GitDir
        Install-aura_intelligenceFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate
    } else {
        Install-aura_intelligence
    }

    if (-not (Ensure-aura_intelligenceOnPath)) {
        Write-Host "Install completed, but aura_intelligence is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: aura_intelligence doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (aura_intelligence --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.aura_intelligence -and $npmList.dependencies.aura_intelligence.version) {
                $installedVersion = $npmList.dependencies.aura_intelligence.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "aura_intelligence installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "aura_intelligence installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\aura_intelligence.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "aura_intelligence doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "aura_intelligence onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
