...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "aura_intelligence")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "aura_intelligence")
...
DMG_TEMP="$(mktemp -d /tmp/aura_intelligence-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/aura_intelligence/Resources/aura_intelligence.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/aura_intelligence-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/aura_intelligence-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/aura_intelligence-dmg-limits.txt
