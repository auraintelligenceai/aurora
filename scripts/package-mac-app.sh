...
# Build and bundle aura_intelligence into a minimal .app we can open.
# Outputs to dist/aura_intelligence.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/aura_intelligence.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="aura_intelligence"
BUNDLE_ID="${BUNDLE_ID:-aura.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/aura_intelligence/aura_intelligence/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/aura_intelligence/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :aura_intelligenceBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :aura_intelligenceGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/aura_intelligence"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/aura_intelligence"
fi
chmod +x "$APP_ROOT/Contents/MacOS/aura_intelligence"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/aura_intelligence" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/aura_intelligence/Resources/aura_intelligence.icns" "$APP_ROOT/Contents/Resources/aura_intelligence.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/aura_intelligence/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying aura_intelligenceKit resources"
aura_intelligenceKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/aura_intelligenceKit_aura_intelligenceKit.bundle"
if [ -d "$aura_intelligenceKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/aura_intelligenceKit_aura_intelligenceKit.bundle"
  cp -R "$aura_intelligenceKIT_BUNDLE" "$APP_ROOT/Contents/Resources/aura_intelligenceKit_aura_intelligenceKit.bundle"
else
  echo "WARN: aura_intelligenceKit resource bundle not found at $aura_intelligenceKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running aura_intelligence"
killall -q aura_intelligence 2>/dev/null || true
