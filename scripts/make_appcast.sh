...
ZIP=${1:?"Usage: $0 aura_intelligence-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/aura_intelligence/aura_intelligence/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^aura_intelligence-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/aura_intelligence/aura_intelligence/releases/download/v${VERSION}/"}
