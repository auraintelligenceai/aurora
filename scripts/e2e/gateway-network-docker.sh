...
IMAGE_NAME="aura_intelligence-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="aura_intelligence-net-e2e-$$"
GW_NAME="aura_intelligence-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "AURA_GATEWAY_TOKEN=$TOKEN" \
	  -e "AURA_SKIP_CHANNELS=1" \
	  -e "AURA_SKIP_GMAIL_WATCHER=1" \
	  -e "AURA_SKIP_CRON=1" \
	  -e "AURA_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
