...
IMAGE_NAME="aura_intelligence-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/aura_intelligence-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/aura_intelligence-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/aura_intelligence-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/aura_intelligence-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/aura_intelligence-e2e-$1.XXXXXX"
  }
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.aura/aura_intelligence.json" <<'"'"'JSON'"'"'
{
