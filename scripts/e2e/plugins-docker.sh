...
IMAGE_NAME="aura_intelligence-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/aura_intelligence-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.aura/extensions"

  cat > "$HOME/.aura/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/aura_intelligence-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@aura_intelligence/demo-plugin-tgz",
  "version": "0.0.1",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/aura_intelligence-plugin-dir.XXXXXX")"
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/aura_intelligence-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@aura_intelligence/demo-plugin-file",
  "version": "0.0.1",
  "aura_intelligence": { "extensions": ["./index.js"] }
}
JSON
