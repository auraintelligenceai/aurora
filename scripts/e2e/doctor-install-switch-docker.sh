...
IMAGE_NAME="aura_intelligence-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/aura_intelligence-bin:$PATH"
  mkdir -p /tmp/aura_intelligence-bin

  cat > /tmp/aura_intelligence-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/aura_intelligence-bin/systemctl

  cat > /tmp/aura_intelligence-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/aura_intelligence-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/aura_intelligence"
  npm_entry="/tmp/npm-prefix/lib/node_modules/aura_intelligence/aura_intelligence.mjs"
  git_entry="/app/aura_intelligence.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/aura_intelligence-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/aura_intelligence-gateway.service"
    if [ ! -f "$unit_path" ]; then
