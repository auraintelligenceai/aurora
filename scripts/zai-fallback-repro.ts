...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "aura_intelligence-zai-fallback-"),
  );
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "aura_intelligence.json");
  ...
  const sessionId =
    process.env.AURA_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
