...
BASE_IMAGE="${BASE_IMAGE:-aura_intelligence-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-aura_intelligence-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=aura_intelligence.sandbox=1)
NOTE
