...
const compiler = env.AURA_TS_COMPILER === "tsgo" ? "tsgo" : "tsc";
...
const shouldBuild = () => {
  if (env.AURA_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.AURA_RUNNER_LOG === "0") return;
  process.stderr.write(`[aura_intelligence] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["aura_intelligence.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
