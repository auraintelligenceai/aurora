#!/bin/bash
# Setup aura_intelligence Auth Management System
...
echo "=== aura_intelligence Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'aura_intelligence-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  aura_intelligence message: Send warning via aura_intelligence itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/aura_intelligence-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/aura_intelligence-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/aura_intelligence-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now aura_intelligence-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status aura_intelligence-auth-monitor.timer"
echo ""
echo "Quick commands:"
