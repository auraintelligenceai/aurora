#!/usr/bin/env bash
# Reset aura_intelligence like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node aura_intelligence.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.aura/disable-launchagent"
      log ""
...
log "==> Killing existing aura_intelligence instances"
kill_all_aura_intelligence
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product aura_intelligence"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/aura_intelligence.app" ]]; then
    APP_BUNDLE="/Applications/aura_intelligence.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/aura_intelligence.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/aura_intelligence.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/aura_intelligence.app missing Sparkle after packaging"
    fi
    return 0
  fi
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node aura_intelligence.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node aura_intelligence.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".aura", "aura_intelligence.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: aura_intelligence is running."
else
