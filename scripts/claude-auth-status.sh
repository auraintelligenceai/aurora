...
# Checks both Claude Code and aura_intelligence auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
AURA_AUTH="$HOME/.aura/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    aura_intelligence models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    aura_intelligence_status=$(check_aura_intelligence_auth 2>/dev/null || true)

    claude_expires=0
    aura_intelligence_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        aura_intelligence_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        aura_intelligence_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$AURA_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$aura_intelligence_status" \
        --arg be "$aura_intelligence_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            aura_intelligence: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$aura_intelligence_status" == EXPIRED* ]] || [[ "$aura_intelligence_status" == MISSING* ]]; then
        echo "AURA_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$aura_intelligence_status" == EXPIRING* ]]; then
        echo "AURA_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "aura_intelligence Auth (~/.aura/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$AURA_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$AURA_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'aura_intelligence doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active aura_intelligence >/dev/null 2>&1; then
    echo -e "aura_intelligence service: ${GREEN}running${NC}"
else
    echo -e "aura_intelligence service: ${RED}NOT running${NC}"
fi
