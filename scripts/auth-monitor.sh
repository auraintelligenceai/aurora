...
#
# Suggested cron: */30 * * * * /home/admin/aura_intelligence/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send aura_intelligence notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., aura_intelligence-alerts)
...
STATE_FILE="$HOME/.aura/auth-monitor-state"
...
send_notification() {
    ...
    # Send via aura_intelligence if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use aura_intelligence
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via aura_intelligence to $NOTIFY_PHONE..."
            aura_intelligence send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
    ...
    if [ -n "$NOTIFY_NTFY" ]; then
        ...
        curl -s -o /dev/null \
            -H "Title: aura_intelligence Auth Alert" \
            -H "Priority: $priority" \
            -H "Tags: warning,key" \
            -d "$message" \
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! aura_intelligence is down. Run: ssh l36 '~/aura_intelligence/scripts/mobile-reauth.sh'" "urgent"
    exit 1
