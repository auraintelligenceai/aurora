...
export async function handleWhatsAppStart(host: aura_intelligenceApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: aura_intelligenceApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: aura_intelligenceApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: aura_intelligenceApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: aura_intelligenceApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: aura_intelligenceApp): string {
...
export function handleNostrProfileCancel(host: aura_intelligenceApp) {
...
export function handleNostrProfileToggleAdvanced(host: aura_intelligenceApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: aura_intelligenceApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: aura_intelligenceApp) {
  const state = host.nostrProfileFormState;
