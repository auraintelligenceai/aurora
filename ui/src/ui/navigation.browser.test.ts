...
beforeEach(() => {
  ...
  window.__AURA_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/aura_intelligence/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/aura_intelligence");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/aura_intelligence/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AURA_CONTROL_UI_BASE_PATH__ = "/aura_intelligence";
    const app = mountApp("/aura_intelligence/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/aura_intelligence");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/aura_intelligence/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem(
      "aura_intelligence.control.settings.v1",
