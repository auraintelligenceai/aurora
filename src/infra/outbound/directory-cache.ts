...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: aura_intelligenceConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: aura_intelligenceConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: aura_intelligenceConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: aura_intelligenceConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: aura_intelligenceConfig): void {
