...
export function triggeraura_intelligenceRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.AURA_SYSTEMD_UNIT,
        process.env.AURA_PROFILE,
      );
  ...
  const label =
    process.env.AURA_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.AURA_PROFILE);
