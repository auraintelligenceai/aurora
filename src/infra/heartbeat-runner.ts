...
function resolveActiveHoursTimezone(cfg: aura_intelligenceConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: aura_intelligenceConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: aura_intelligenceConfig) {
...
function resolveHeartbeatAgents(cfg: aura_intelligenceConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: aura_intelligenceConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: aura_intelligenceConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: aura_intelligenceConfig) => {
    if (state.stopped) return;
    const now = Date.now();
