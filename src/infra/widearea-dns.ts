...
export const WIDE_AREA_DISCOVERY_DOMAIN = "aura_intelligence.internal.";
export const WIDE_AREA_ZONE_FILENAME = "aura_intelligence.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*aura_intelligence-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  ...
  return `; aura_intelligence-content-hash: ${contentHash}\n${contentBody}`;
}
