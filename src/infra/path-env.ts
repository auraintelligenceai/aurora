...
type Ensureaura_intelligencePathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: Ensureaura_intelligencePathOpts): string[] {
  ...
  // Bundled macOS app: `aura_intelligence` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingaura_intelligence = path.join(execDir, "aura_intelligence");
    if (isExecutable(siblingaura_intelligence)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/aura_intelligence` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "aura_intelligence"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `aura_intelligence` CLI can run
