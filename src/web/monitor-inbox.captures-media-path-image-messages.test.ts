...
describe("web monitor inbox", () => {
  beforeEach(() => {
    ...
    authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "aura_intelligence-auth-"));
  });
  ...
  it("logs inbound bodies to file", async () => {
    const logPath = path.join(os.tmpdir(), `aura_intelligence-log-test-${crypto.randomUUID()}.log`);
