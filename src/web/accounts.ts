...
export function listWhatsAppAuthDirs(cfg: aura_intelligenceConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: aura_intelligenceConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: aura_intelligenceConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: aura_intelligenceConfig): string {
