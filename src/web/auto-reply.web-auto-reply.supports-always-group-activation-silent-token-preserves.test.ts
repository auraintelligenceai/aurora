...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-session-"));
...
describe("web auto-reply", () => {
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/aura_intelligence-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/aura_intelligence-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
