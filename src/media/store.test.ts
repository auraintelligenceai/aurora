...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "AURA_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.AURA_STATE_DIR = path.join(home, ".aura");
    ...
    await fs.mkdir(path.join(home, ".aura"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.aura${path.sep}media`);
