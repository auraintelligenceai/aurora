...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "aura_intelligence";
  if (/aura_intelligence/i.test(trimmed)) return trimmed;
  return `${trimmed} (aura_intelligence)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.AURA_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "aura_intelligence");
  ...
  const binCli = path.join(cwd, "bin", "aura_intelligence.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.AURA_TAILNET_DNS?.trim();
