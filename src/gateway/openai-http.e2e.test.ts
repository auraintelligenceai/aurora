    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "aura_intelligence", messages: [{ role: "user", content: "hi" }] },
          { "x-aura_intelligence-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-aura_intelligence-agent-id": "alpha" },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-aura_intelligence-agent-id": "beta",
            "x-aura_intelligence-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
