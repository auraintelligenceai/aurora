...
export async function startGatewayServer(
  ...
  process.env.AURA_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "AURA_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "AURA_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("aura_intelligence doctor")}" to migrate.`,
      );
    }
