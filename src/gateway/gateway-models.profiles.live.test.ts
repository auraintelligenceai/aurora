...
import { resolveaura_intelligenceAgentDir } from "../agents/agent-paths.js";
...
import { ensureaura_intelligenceModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { aura_intelligenceConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.AURA_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.AURA_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.AURA_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: aura_intelligenceConfig;
  agentDir: string;
}): aura_intelligenceConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<aura_intelligenceConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.AURA_AGENT_DIR,
  ...
  const hostAgentDir = resolveaura_intelligenceAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-live-state-"));
  ...
  process.env.AURA_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.aura-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveaura_intelligenceAgentDir();
  const sanitizedCfg: aura_intelligenceConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-live-"));
  const tempConfigPath = path.join(tempDir, "aura_intelligence.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.AURA_CONFIG_PATH = tempConfigPath;

  await ensureaura_intelligenceModelsJson(nextCfg);
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "aura_intelligence live tool probe (local, safe): " +
    ...
    process.env.AURA_CONFIG_PATH = previous.configPath;
    process.env.AURA_GATEWAY_TOKEN = previous.token;
    process.env.AURA_SKIP_CHANNELS = previous.skipChannels;
    process.env.AURA_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.AURA_SKIP_CRON = previous.skipCron;
    process.env.AURA_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.AURA_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.AURA_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureaura_intelligenceModelsJson(cfg);
      ...
      const rawModels = process.env.AURA_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.aura-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
