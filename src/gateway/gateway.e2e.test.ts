...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.AURA_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-gw-mock-home-"));
      ...
      const toolProbePath = path.join(workspaceDir, `.aura-tool-probe.${nonceA}.txt`);
      ...
      try {
        ...
        process.env.AURA_CONFIG_PATH = prev.configPath;
        process.env.AURA_GATEWAY_TOKEN = prev.token;
        process.env.AURA_SKIP_CHANNELS = prev.skipChannels;
        process.env.AURA_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.AURA_SKIP_CRON = prev.skipCron;
        process.env.AURA_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.AURA_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.AURA_STATE_DIR = prev.stateDir;
      process.env.AURA_CONFIG_PATH = prev.configPath;
      process.env.AURA_GATEWAY_TOKEN = prev.token;
      process.env.AURA_SKIP_CHANNELS = prev.skipChannels;
      process.env.AURA_SKIP_GMAIL_WATCHER = prev.skipGmail;
