...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.AURA_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.AURA_GATEWAY_PORT = String(gatewayPort);
  process.env.AURA_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.AURA_GATEWAY_TOKEN = prevGatewayToken;
  }
});
