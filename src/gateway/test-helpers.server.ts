...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.AURA_STATE_DIR;
  previousConfigPath = process.env.AURA_CONFIG_PATH;
  previousSkipBrowserControl = process.env.AURA_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.AURA_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.AURA_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.AURA_STATE_DIR = path.join(tempHome, ".aura");
  delete process.env.AURA_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.AURA_SKIP_BROWSER_CONTROL_SERVER = "1";
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.AURA_STATE_DIR;
    else process.env.AURA_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.AURA_CONFIG_PATH;
    else process.env.AURA_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.AURA_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.AURA_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.AURA_SKIP_GMAIL_WATCHER;
    else process.env.AURA_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.AURA_SKIP_CANVAS_HOST;
    else process.env.AURA_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.AURA_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.AURA_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.AURA_GATEWAY_PASSWORD;
