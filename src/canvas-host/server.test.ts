...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("aura_intelligenceCanvasA2UIAction");
    expect(out).toContain("aura_intelligenceSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("aura_intelligenceSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.aura_intelligenceA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__aura_intelligence__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("aura_intelligence-a2ui-host");
      expect(html).toContain("aura_intelligenceCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__aura_intelligence__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("aura_intelligenceA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
