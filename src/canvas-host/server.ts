...
<title>aura_intelligence Canvas</title>
<style>
...
<div class="wrap">
  <div class="card">
    <div class="title">
      <h1>aura_intelligence Canvas</h1>
...
(() => {
  ...
  window.addEventListener("aura_intelligence:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android aura_intelligence node canvas.");
      return;
    }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>aura_intelligence Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
