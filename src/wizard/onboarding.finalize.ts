...
type FinalizeOnboardingOptions = {
  flow: WizardFlow;
  opts: OnboardOptions;
  baseConfig: aura_intelligenceConfig;
    ...
    if (hatchChoice === "tui") {
      ...
      if (seededInBackground) {
        await prompter.note(
          `Web UI seeded in the background. Open later with: ${formatCliCommand(
            "aura_intelligence dashboard --no-open",
          )}`,
          "Web UI",
        );
      }
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control aura_intelligence."
            : "Copy/paste this URL in a browser on this machine to control aura_intelligence.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
