...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "If you’re not comfortable with basic security and access control, don’t run aura_intelligence.",
...
export async function runOnboardingWizard(
  ...
  let baseConfig: aura_intelligenceConfig = snapshot.valid ? snapshot.config : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("aura_intelligence doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("aura_intelligence configure")}.`;
  ...
  let nextConfig: aura_intelligenceConfig = {
    ...baseConfig,
    agents: {
      ...baseConfig.agents,
      defaults: {
