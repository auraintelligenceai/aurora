    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aura_intelligence status", "Show channel health + session summary."],
          ["aura_intelligence status --all", "Full diagnosis (read-only)."],
          ["aura_intelligence status --json", "Machine-readable output."],
          ["aura_intelligence status --usage", "Show model provider usage/quota snapshots."],
          [
            "aura_intelligence status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["aura_intelligence status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
