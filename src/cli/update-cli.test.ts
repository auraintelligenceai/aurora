...
vi.mock("../infra/aura_intelligence-root.js", () => ({
  resolveaura_intelligencePackageRoot: vi.fn(),
}));
...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(resolveaura_intelligencePackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("aura_intelligence update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveaura_intelligencePackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
