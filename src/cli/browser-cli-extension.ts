      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “aura_intelligence Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand("aura_intelligence browser extension install")}"`,
