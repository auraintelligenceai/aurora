...
const STEP_LABELS: Record<string, string> = {
  ...
  "aura_intelligence doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "aura_intelligence";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "aura_intelligence"]);
const CLI_NAME = resolveCliName();
const AURA_REPO_URL = "https://github.com/aura_intelligence/aura_intelligence.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "aura_intelligence");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("aura_intelligence@")) return trimmed.slice("aura_intelligence@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.AURA_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AURA_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `AURA_GIT_DIR points at a non-git directory: ${params.dir}. Set AURA_GIT_DIR to an empty folder or a aura_intelligence checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AURA_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveaura_intelligencePackageRoot({
  ...
  defaultRuntime.log(theme.heading("aura_intelligence update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating aura_intelligence..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this aura_intelligence install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("aura_intelligence doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("aura_intelligence gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g aura_intelligence@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g aura_intelligence@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("aura_intelligence gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aura_intelligence doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("aura_intelligence gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aura_intelligence gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `aura_intelligence update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `AURA_GIT_DIR points at a non-git directory: ${gitDir}. Set AURA_GIT_DIR to an empty folder or a aura_intelligence checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via AURA_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update aura_intelligence to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["aura_intelligence update", "Update a source checkout (git)"],
        ["aura_intelligence update --channel beta", "Switch to beta channel (git + npm)"],
        ["aura_intelligence update --channel dev", "Switch to dev channel (git + npm)"],
        ["aura_intelligence update --tag beta", "One-off update to a dist-tag or version"],
        ["aura_intelligence update --no-restart", "Update without restarting the service"],
        ["aura_intelligence update --json", "Output result as JSON"],
        ["aura_intelligence update --yes", "Non-interactive (accept downgrade prompts)"],
        ["aura_intelligence update wizard", "Interactive update wizard"],
        ["aura_intelligence --update", "Shorthand for aura_intelligence update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run aura_intelligence update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.molt.bot/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aura_intelligence update status", "Show channel + version status."],
          ["aura_intelligence update status --json", "JSON output."],
          ["aura_intelligence update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
