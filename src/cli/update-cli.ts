...
const CLAWDBOT_REPO_URL = "https://github.com/aura_intelligence/aura_intelligence.git";
...
async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `CLAWDBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set CLAWDBOT_GIT_DIR to an empty folder or a aura_intelligence checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `CLAWDBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set CLAWDBOT_GIT_DIR to an empty folder or a aura_intelligence checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
