...
function appendClawdHubHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: use \`npx aurahub\` to search, install, and sync skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("aura_intelligence skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendClawdHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("aura_intelligence skills list")}\` to see available skills.`,
      opts.json,
    );
  }
