...
export const DEFAULT_CLI_NAME = "aura_intelligence";
export const LEGACY_CLI_NAME = "aura_intelligence";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME, LEGACY_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(aura_intelligence|aura_intelligence)\b/;

export function resolveCliName(
  argv: string[] = process.argv,
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string {
  const override = env.aura_intelligence_CLI_NAME?.trim() || env.CLAWDBOT_CLI_NAME?.trim();
