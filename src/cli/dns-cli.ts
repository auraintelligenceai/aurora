...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      const serverPath = path.join(confDir, "aura_intelligence.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by aura_intelligence dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
