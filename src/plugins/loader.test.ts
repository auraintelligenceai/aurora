...
import { loadaura_intelligencePlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.AURA_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `aura_intelligence-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "aura_intelligence.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.AURA_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadaura_intelligencePlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.AURA_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadaura_intelligencePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadaura_intelligencePlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@aura_intelligence/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        aura_intelligence: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
