...
type PackageManifest = {
  ...
  aura_intelligence?: { extensions?: string[] };
...
async function ensureaura_intelligenceExtensions(manifest: PackageManifest) {
  ...
  if (list.length === 0) {
    throw new Error("package.json aura_intelligence.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureaura_intelligenceExtensions(manifest);
  } catch (err) {
...
export async function installPluginFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-plugin-"));
...
export async function installPluginFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-npm-pack-"));
