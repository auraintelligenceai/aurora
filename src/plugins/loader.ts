...
import { discoveraura_intelligencePlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: aura_intelligencePluginDefinition;
  register?: aura_intelligencePluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as aura_intelligencePluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as aura_intelligencePluginDefinition;
...
export function loadaura_intelligencePlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoveraura_intelligencePlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: aura_intelligencePluginModule | null = null;
    try {
      mod = jiti(candidate.source) as aura_intelligencePluginModule;
    } catch (err) {
