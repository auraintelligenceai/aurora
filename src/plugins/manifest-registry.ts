...
import { discoveraura_intelligencePlugins, type PluginCandidate } from "./discovery.js";
...
function resolveManifestCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AURA_PLUGIN_MANIFEST_CACHE_MS?.trim();
...
function shouldUseManifestCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.AURA_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
...
export function loadPluginManifestRegistry(params: {
  ...
  const discovery = params.candidates
    ...
    : discoveraura_intelligencePlugins({
