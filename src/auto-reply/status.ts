...
type AgentConfig = Partial<NonNullable<NonNullable<aura_intelligenceConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.aura/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 aura_intelligence ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: aura_intelligenceConfig): string {
  const lines = ["ℹ️ Help", ""];
