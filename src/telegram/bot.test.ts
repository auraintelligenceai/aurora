...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/aura_intelligence-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
...
describe("createTelegramBot", () => {
  ...
  it("routes callback_query payloads as messages and answers callbacks", async () => {
    ...
    await callbackHandler({
      ...
      me: { username: "aura_intelligence_bot" },
  ...
  it("blocks group messages when telegram.groups is set without a wildcard", async () => {
    ...
    await handler({
      message: {
        ...
        text: "@aura_intelligence_bot hello",
        date: 1736380800,
      },
      me: { username: "aura_intelligence_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "aura_intelligence" },
        },
      },
      me: { id: 999, username: "aura_intelligence_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("honors routed group activation from session store", async () => {
    ...
    const replySpy = replyModule.__replySpy as unknown as ReturnType<typeof vi.fn>;
    replySpy.mockReset();
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "aura_intelligence-telegram-"));
