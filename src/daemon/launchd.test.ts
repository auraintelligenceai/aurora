...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.AURA_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.AURA_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-launchctl-test-"));
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.AURA_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env.AURA_TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env.AURA_TEST_LAUNCHCTL_LOG = logPath;
    process.env.AURA_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    process.env.PATH = `${binDir}${path.delimiter}${originalPath ?? ""}`;

    await run({
      env: {
        HOME: homeDir,
        AURA_PROFILE: "default",
      },
      logPath,
    });
  } finally {
    ...
    if (originalLogPath === undefined) {
      delete process.env.AURA_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.AURA_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.AURA_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.AURA_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    await withLaunchctlStub({ listOutput: "123 0 aura.gateway\n" }, async ({ env }) => {
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when AURA_PROFILE is default", () => {
    const env = { HOME: "/Users/test", AURA_PROFILE: "default" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/aura.gateway.plist",
    );
  });

  it("uses default label when AURA_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when AURA_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", AURA_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/aura.jbphoenix.plist",
    );
  });

  it("prefers AURA_LAUNCHD_LABEL over AURA_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      AURA_PROFILE: "jbphoenix",
      AURA_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from AURA_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      AURA_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty AURA_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      AURA_PROFILE: "myprofile",
      AURA_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/aura.myprofile.plist",
    );
  });
  ...
  it("trims whitespace from AURA_PROFILE", () => {
    const env = { HOME: "/Users/test", AURA_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/aura.myprofile.plist",
    );
  });
