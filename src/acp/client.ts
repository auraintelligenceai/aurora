...
export async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  ensureaura_intelligenceCliOnPath({ cwd });
  const serverCommand = opts.serverCommand ?? "aura_intelligence";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "aura_intelligence-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("aura_intelligence ACP client");
