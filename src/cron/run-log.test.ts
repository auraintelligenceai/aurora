...
describe("cron run log", () => {
  ...
  it("appends JSONL and prunes by line count", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-cron-log-"));
  ...
  it("reads newest entries and filters by jobId", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-cron-log-read-"));
