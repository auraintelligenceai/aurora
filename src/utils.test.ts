...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "aura_intelligence-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "aura_intelligence-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "aura_intelligence-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.aura_intelligence when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-config-dir-"));
    try {
      const newDir = path.join(root, ".aura_intelligence");
