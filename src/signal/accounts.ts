...
export function listSignalAccountIds(cfg: aura_intelligenceConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: aura_intelligenceConfig): string {
...
export function listEnabledSignalAccounts(cfg: aura_intelligenceConfig): ResolvedSignalAccount[] {
