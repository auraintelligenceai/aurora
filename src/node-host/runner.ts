...
import { ensureaura_intelligenceCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.AURA_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.AURA_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureaura_intelligenceCliOnPath({ pathEnv: process.env.PATH ?? "" });
