...
import type { aura_intelligenceConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareaura_intelligenceVersions } from "./version.js";
...
function coerceConfig(value: unknown): aura_intelligenceConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as aura_intelligenceConfig;
}
...
function stampConfigVersion(cfg: aura_intelligenceConfig): aura_intelligenceConfig {
...
function warnIfConfigFromFuture(
  ...
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer aura_intelligence (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: aura_intelligenceConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): aura_intelligenceConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as aura_intelligenceConfig, deps.env);
      }
  ...
  async function writeConfigFile(cfg: aura_intelligenceConfig) {
    clearConfigCache();
...
// module scope. `AURA_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.AURA_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.AURA_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export function loadConfig(): aura_intelligenceConfig {
...
export async function writeConfigFile(cfg: aura_intelligenceConfig): Promise<void> {
  clearConfigCache();
