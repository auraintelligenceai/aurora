...
import type { aura_intelligenceConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: aura_intelligenceConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: aura_intelligenceConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = aura_intelligenceSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as aura_intelligenceConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as aura_intelligenceConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as aura_intelligenceConfig)),
    ),
  };
}
