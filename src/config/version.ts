export type aura_intelligenceVersion = {
  major: number;
  minor: number;
  patch: number;
  revision: number;
...
export function compareaura_intelligenceVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseaura_intelligenceVersion(a);
  const parsedB = parseaura_intelligenceVersion(b);
