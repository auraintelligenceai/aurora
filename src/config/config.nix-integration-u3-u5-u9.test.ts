...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    ...
    it("CONFIG_PATH defaults to ~/.clawdbot/aura_intelligence.json when env not set", async () => {
      await withEnvOverride(
        {
        ...
        async () => {
          ...
          expect(CONFIG_PATH).toMatch(/\.clawdbot[\\/]aura_intelligence\.json$/);
        },
      );
    });
    ...
    it("CONFIG_PATH respects CLAWDBOT_CONFIG_PATH override", async () => {
      await withEnvOverride(
        { aura_intelligence_CONFIG_PATH: undefined, CLAWDBOT_CONFIG_PATH: "/nix/store/abc/aura_intelligence.json" },
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/abc/aura_intelligence.json"));
        },
      );
    });

    it("CONFIG_PATH prefers aura_intelligence_CONFIG_PATH over legacy override", async () => {
      await withEnvOverride(
        {
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/new/aura_intelligence.json"));
        },
      );
    });
    ...
    it("CONFIG_PATH expands ~ in CLAWDBOT_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        await withEnvOverride(
          { aura_intelligence_CONFIG_PATH: undefined, CLAWDBOT_CONFIG_PATH: "~/.clawdbot/custom.json" },
          async () => {
            const { CONFIG_PATH } = await import("./config.js");
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.join(path.resolve("/custom/state"), "aura_intelligence.json"));
        },
      );
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        ...
        await fs.writeFile(
          path.join(pluginDir, "aura_intelligence.plugin.json"),
          JSON.stringify(
            {
              id: "demo-plugin",
