...
describe("state + config path candidates", () => {
  ...
  it("prefers ~/.aura_intelligence when it exists and legacy dir is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-state-"));
    try {
  ...
  it("CONFIG_PATH prefers existing legacy filename when present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-config-"));
  ...
  it("respects state dir overrides when config is missing", async () => {
    ...
    try {
      ...
      const env = { aura_intelligence_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const resolved = resolveConfigPath(env, overrideDir, () => root);
      expect(resolved).toBe(path.join(overrideDir, "aura_intelligence.json"));
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
