...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-aura",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-aura#general");
  });
  ...
  it("derives session transcripts dir from AURA_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { AURA_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.AURA_STATE_DIR = "/custom/state";
    try {
