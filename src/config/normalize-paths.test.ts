  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.aura/logs/aura_intelligence.log" },
        hooks: {
          path: "~/.aura/hooks.json5",
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".aura", "logs", "aura_intelligence.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".aura", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".aura", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".aura", "agents", "main"));
