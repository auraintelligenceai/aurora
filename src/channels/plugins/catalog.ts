...
import { discoveraura_intelligencePlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { aura_intelligencePackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  aura_intelligence?: aura_intelligencePackageManifest;
  [LEGACY_MANIFEST_KEY]?: aura_intelligencePackageManifest;
};
...
const ENV_CATALOG_PATHS = ["AURA_PLUGIN_CATALOG_PATHS", "AURA_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<aura_intelligencePackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: aura_intelligencePackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageaura_intelligence?: aura_intelligencePackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageaura_intelligence;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.aura_intelligence ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageaura_intelligence: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoveraura_intelligencePlugins({ workspaceDir: options.workspaceDir });
