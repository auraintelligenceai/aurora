...
function setSignalDmPolicy(cfg: aura_intelligenceConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "aura_intelligence"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("aura_intelligence gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
