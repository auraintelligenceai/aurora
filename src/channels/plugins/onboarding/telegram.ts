...
function setTelegramDmPolicy(cfg: aura_intelligenceConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramTokenHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Website: https://github.com/auraintelligenceai/aurora",
    ].join("\n"),
    "Telegram bot token",
  );
}
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("aura_intelligence logs --follow")}\` (safest)`,
