...
function setSlackDmPolicy(cfg: aura_intelligenceConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "aura_intelligence";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for aura_intelligence`,
    },
    features: {
      bot_user: {
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds }) => {
    ...
    const slackBotName = String(
      await prompter.text({
        ...
        initialValue: "aura_intelligence",
      }),
    ).trim();
