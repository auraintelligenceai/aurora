...
export function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`aura_intelligence pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`aura_intelligence pairing approve ${channelId} <code>`);
