...
declare const __AURA_VERSION__: string | undefined;
...
async function main() {
  ...
  const { ensureaura_intelligenceCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[aura_intelligence] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
