...
const BUNDLED_VERSION =
  (typeof __AURA_VERSION__ === "string" && __AURA_VERSION__) ||
  process.env.AURA_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `aura_intelligence --version` behavior for Swift env/version checks.
...
void main().catch((err) => {
  console.error(
    "[aura_intelligence] Gateway daemon failed:",
