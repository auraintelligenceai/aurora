...
const ensureaura_intelligenceModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveaura_intelligenceAgentDir = vi.fn().mockReturnValue("/tmp/aura_intelligence-agent");
const ensureAuthProfileStore = vi.fn().mockReturnValue({ version: 1, profiles: {} });
const listProfilesForProvider = vi.fn().mockReturnValue([]);
const resolveAuthProfileDisplayLabel = vi.fn(({ profileId }: { profileId: string }) => profileId);
const resolveAuthStorePathForDisplay = vi
  .fn()
  .mockReturnValue("/tmp/aura_intelligence-agent/auth-profiles.json");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/aura_intelligence-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureaura_intelligenceModelsJson,
}));
