...
function stripUnknownConfigKeys(config: aura_intelligenceConfig): {
  config: aura_intelligenceConfig;
  removed: string[];
} {
  const parsed = aura_intelligenceSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as aura_intelligenceConfig;
...
function noteOpencodeProviderOverrides(cfg: aura_intelligenceConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: aura_intelligenceConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as aura_intelligenceConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("aura_intelligence doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "aura_intelligence doctor --fix" to remove these keys.');
    }
  }
