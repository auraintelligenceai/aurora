...
export async function statusCommand(
  ...
  runtime.log(theme.heading("aura_intelligence status"));
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("aura_intelligence status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("aura_intelligence logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("aura_intelligence status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("aura_intelligence gateway probe")}`);
  }
}
