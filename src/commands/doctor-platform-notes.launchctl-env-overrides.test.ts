...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "AURA_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("AURA_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv AURA_GATEWAY_TOKEN");
    expect(message).not.toContain("AURA_GATEWAY_PASSWORD");
  });
