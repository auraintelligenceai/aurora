...
describe("channels command", () => {
  ...
  it("surfaces Discord privileged intent issues in channels status output", () => {
    ...
    expect(lines.join("\n")).toMatch(/Run: (?:aura_intelligence|aura_intelligence)( --profile isolated)? doctor/);
  });
        ...
        telegram: [
          {
            ...
            enabled: true,
            configured: true,
            probe: { ok: true, bot: { username: "aura_intelligence_bot" } },
          },
        ],
      },
    });
    expect(lines.join("\n")).toMatch(/bot:@aura_intelligence_bot/);
  });
