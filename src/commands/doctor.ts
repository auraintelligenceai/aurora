...
function resolveMode(cfg: aura_intelligenceConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("aura_intelligence doctor");

  const root = await resolveaura_intelligencePackageRoot({
  ...
  let cfg: aura_intelligenceConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("aura_intelligence configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("aura_intelligence config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("aura_intelligence setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("aura_intelligence doctor --fix")}" to apply changes.`);
  }
