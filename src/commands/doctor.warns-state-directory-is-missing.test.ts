...
beforeEach(() => {
  ...
  resolveaura_intelligencePackageRoot.mockReset().mockResolvedValue(null);
  ...
  legacyReadConfigFileSnapshot.mockReset().mockResolvedValue({
    path: "/tmp/aura_intelligence.json",
    exists: false,
    raw: null,
    parsed: {},
    valid: true,
...
const resolveaura_intelligencePackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadaura_intelligencePlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/aura_intelligence.json",
    createConfigIO,
...
vi.mock("../infra/aura_intelligence-root.js", () => ({
  resolveaura_intelligencePackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "aura_intelligence-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.AURA_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when AURA_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/aura_intelligence.json",
      exists: true,
      raw: "{}",
    ...
    process.env.AURA_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
