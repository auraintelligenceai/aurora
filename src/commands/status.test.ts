...
vi.mock("../infra/aura_intelligence-root.js", () => ({
  resolveaura_intelligencePackageRoot: vi.fn().mockResolvedValue("/tmp/aura_intelligence"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/aura_intelligence/pnpm-lock.yaml",
      markerPath: "/tmp/aura_intelligence/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("aura_intelligence status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("aura_intelligence status --all") ||
          l.includes("aura_intelligence --profile isolated status --all") ||
          l.includes("aura_intelligence status --all") ||
          l.includes("aura_intelligence --profile isolated status --all"),
      ),
    ).toBe(true);
