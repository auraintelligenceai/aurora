...
export function listAgentEntries(cfg: aura_intelligenceConfig): AgentEntry[] {
...
function resolveAgentName(cfg: aura_intelligenceConfig, agentId: string) {
...
function resolveAgentModel(cfg: aura_intelligenceConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: aura_intelligenceConfig): AgentSummary[] {
