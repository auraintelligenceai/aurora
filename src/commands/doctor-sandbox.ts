...
function resolveSandboxDockerImage(cfg: aura_intelligenceConfig): string {
  const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
  return image ? image : DEFAULT_SANDBOX_IMAGE;
}

function resolveSandboxBrowserImage(cfg: aura_intelligenceConfig): string {
...
export function noteSandboxScopeWarnings(cfg: aura_intelligenceConfig) {
