...
import type { aura_intelligenceConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: aura_intelligenceConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("aura_intelligence security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("aura_intelligence configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("aura_intelligence config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("aura_intelligence doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("aura_intelligence config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
