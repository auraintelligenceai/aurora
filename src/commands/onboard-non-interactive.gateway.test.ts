...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.AURA_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.AURA_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "aura");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.AURA_STATE_DIR = stateDir;
    process.env.AURA_CONFIG_PATH = path.join(stateDir, "aura_intelligence.json");
