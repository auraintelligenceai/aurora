...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveaura_intelligenceAgentDir: vi.fn().mockReturnValue("/tmp/aura_intelligence-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveaura_intelligenceAgentDir: mocks.resolveaura_intelligenceAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/aura_intelligence-agent/auth-profiles.json");
