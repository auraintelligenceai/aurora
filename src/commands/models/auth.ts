...
import { readConfigFileSnapshot, type aura_intelligenceConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: aura_intelligenceConfig, model: string): aura_intelligenceConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("aura_intelligence plugins install")}\`.`,
    );
  }
