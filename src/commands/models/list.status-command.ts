...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("aura_intelligence models auth setup-token")}\` or \`${formatCliCommand("aura_intelligence configure")}\`.`
          : `Run \`${formatCliCommand("aura_intelligence configure")}\` or set an API key env var.`;
