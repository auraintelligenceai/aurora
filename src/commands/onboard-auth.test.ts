...
const requireAgentDir = () => {
  const agentDir = process.env.AURA_AGENT_DIR;
  if (!agentDir) throw new Error("AURA_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under AURA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-oauth-"));
    process.env.AURA_STATE_DIR = tempStateDir;
    process.env.AURA_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AURA_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AURA_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-minimax-"));
    process.env.AURA_STATE_DIR = tempStateDir;
    process.env.AURA_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AURA_AGENT_DIR;
