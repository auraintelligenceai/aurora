...
import type { aura_intelligencePluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: aura_intelligencePluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: aura_intelligencePluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
...
async function loadHookHandler(
  entry: HookEntry,
  api: aura_intelligencePluginApi,
