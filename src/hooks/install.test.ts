...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `aura_intelligence-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@aura_intelligence/zip-hooks",
        version: "0.0.1",
        aura_intelligence: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@aura_intelligence/tar-hooks",
        version: "0.0.1",
        aura_intelligence: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
