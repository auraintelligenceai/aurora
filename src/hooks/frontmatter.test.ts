...
import {
  parseFrontmatter,
  resolveaura_intelligenceMetadata,
...
`;
    ...
    expect(parsed.aura_intelligence.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.aura_intelligence.emoji).toBe("📝");
    expect(parsed.aura_intelligence.events).toEqual(["command"]);
    expect(parsed.aura_intelligence.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.aura_intelligence.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"aura_intelligence": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"aura_intelligence": {"events": ["test"]}}');
  });
...
describe("resolveaura_intelligenceMetadata", () => {
  it("extracts aura_intelligence metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveaura_intelligenceMetadata(frontmatter);
  ...
  it("returns undefined when aura_intelligence key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
      ...
      metadata: JSON.stringify({
        aura_intelligence: {
          ...
          install: [
            ...
            { id: "npm", kind: "npm", package: "@aura_intelligence/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@aura_intelligence/hook");
  });
...
`;
    ...
    const aura_intelligence = resolveaura_intelligenceMetadata(frontmatter);
    expect(aura_intelligence).toBeDefined();
    expect(aura_intelligence?.emoji).toBe("💾");
    expect(aura_intelligence?.events).toEqual(["command:new"]);
    expect(aura_intelligence?.requires?.config).toEqual(["workspace.dir"]);
    expect(aura_intelligence?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const aura_intelligence = resolveaura_intelligenceMetadata(frontmatter);
    expect(aura_intelligence?.emoji).toBe("disk");
    expect(aura_intelligence?.events).toEqual(["command:new"]);
  });
});
