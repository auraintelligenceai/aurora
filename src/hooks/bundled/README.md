...
```bash
aura hooks enable session-memory
```
...
```bash
aura hooks enable command-logger
```
...
metadata:
  { "aura": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
```bash
aura hooks info session-memory
```
...
```bash
aura hooks enable session-memory
aura hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.aura/aura.json`:
...
2. Restart gateway: `pkill -9 -f 'aura.*gateway' && pnpm aura gateway`
3. Enable the hook: `aura hooks enable my-hook`
