...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): aura_intelligenceConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-workspace-"));
    try {
