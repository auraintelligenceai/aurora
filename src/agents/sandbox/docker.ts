...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "aura_intelligence.sandbox=1");
  args.push("--label", `aura_intelligence.sessionKey=${params.scopeKey}`);
  args.push("--label", `aura_intelligence.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `aura_intelligence.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "aura_intelligence.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`aura_intelligence sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`aura_intelligence sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("aura_intelligence sandbox recreate --all");
}
