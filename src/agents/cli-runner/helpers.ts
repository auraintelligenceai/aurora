...
/**
 * Cleanup suspended aura_intelligence CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: aura_intelligenceConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-cli-images-"));
  const paths: string[] = [];
