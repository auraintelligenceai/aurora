...
import { createaura_intelligenceTools } from "./aura_intelligence-tools.js";
import { __testing, createaura_intelligenceCodingTools } from "./pi-tools.js";
...
describe("createaura_intelligenceCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createaura_intelligenceTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createaura_intelligenceCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "aura_intelligence-outside.txt");
