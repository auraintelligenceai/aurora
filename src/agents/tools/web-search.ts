...
type WebSearchConfig = NonNullable<aura_intelligenceConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: aura_intelligenceConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("aura_intelligence configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.auraintelligence.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://auraintelligence.ai",
      "X-Title": "aura_intelligence Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
