...
type AgentEntry = NonNullable<NonNullable<aura_intelligenceConfig["agents"]>["list"]>[number];
...
function listAgents(cfg: aura_intelligenceConfig): AgentEntry[] {
  const list = cfg.agents?.list;
  if (!Array.isArray(list)) return [];
  return list.filter((entry): entry is AgentEntry => Boolean(entry && typeof entry === "object"));
}

export function listAgentIds(cfg: aura_intelligenceConfig): string[] {
...
export function resolveDefaultAgentId(cfg: aura_intelligenceConfig): string {
  const agents = listAgents(cfg);
  if (agents.length === 0) return DEFAULT_AGENT_ID;
...
function resolveAgentEntry(cfg: aura_intelligenceConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentWorkspaceDir(cfg: aura_intelligenceConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
