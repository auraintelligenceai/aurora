...
const defaultTools = createaura_intelligenceCodingTools();

describe("createaura_intelligenceCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createaura_intelligenceCodingTools();
    ...
    try {
      ...
      const contents = "Hello from aura_intelligence read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "aura_intelligence-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "aura_intelligence-workspace"),
    ...
    const tools = createaura_intelligenceCodingTools({ sandbox });
