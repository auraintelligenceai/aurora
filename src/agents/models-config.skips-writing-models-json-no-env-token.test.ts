...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "aura_intelligence-models-" });
}

const MODELS_CONFIG: aura_intelligenceConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureaura_intelligenceModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureaura_intelligenceModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureaura_intelligenceModelsJson } = await import("./models-config.js");
      const { resolveaura_intelligenceAgentDir } = await import("./agent-paths.js");

      await ensureaura_intelligenceModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveaura_intelligenceAgentDir(), "models.json");
  ...
  it("adds minimax provider when MINIMAX_API_KEY is set", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        await ensureaura_intelligenceModelsJson({});
