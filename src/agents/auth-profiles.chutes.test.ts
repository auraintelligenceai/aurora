...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.AURA_AGENT_DIR;
    else process.env.AURA_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-chutes-"));
    process.env.AURA_STATE_DIR = tempDir;
    process.env.AURA_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
