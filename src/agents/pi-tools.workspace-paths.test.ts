...
import { createaura_intelligenceCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("aura_intelligence-ws-", async (workspaceDir) => {
      await withTempDir("aura_intelligence-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createaura_intelligenceCodingTools({ workspaceDir });
      ...
      await withTempDir("aura_intelligence-cwd-", async (otherDir) => {
        ...
        try {
          ...
          await editTool?.execute("ws-edit", {
            path: testFile,
            oldText: "world",
            newText: "aura_intelligence",
          });

          const updated = await fs.readFile(path.join(workspaceDir, testFile), "utf8");
          expect(updated).toBe("hello aura_intelligence");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("aura_intelligence-ws-", async (workspaceDir) => {
      await withTempDir("aura_intelligence-override-", async (overrideDir) => {
        const tools = createaura_intelligenceCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("aura_intelligence-sandbox-", async (sandboxDir) => {
      await withTempDir("aura_intelligence-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "aura_intelligence-sbx-test",
        ...
        const tools = createaura_intelligenceCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
