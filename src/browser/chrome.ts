...
export async function launchClawdChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`🦞 aura browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`aura browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`aura browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 aura browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
