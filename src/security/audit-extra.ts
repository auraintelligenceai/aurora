...
import type { aura_intelligenceConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: aura_intelligenceConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function collectModels(cfg: aura_intelligenceConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function isClaude45OrHigher(id: string): boolean {
  // Match claude-*-4-5, claude-*-45, claude-*4.5, claude-3-7, claude-3.7, or opus-4-5/opus-45 variants
  // Examples that should match:
  //   claude-opus-4-5, claude-opus-45, claude-4.5, venice/claude-opus-45,
  //   claude-3-7, claude-3.7, claude-sonnet-4-6, claude-sonnet-4.6
  return /\bclaude-[^\s/]*?(?:-4-?[56]\b|4\.[56]\b|-3-?7\b|3\.7\b)/i.test(id);
}

export function collectModelHygieneFindings(cfg: aura_intelligenceConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: aura_intelligenceConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: aura_intelligenceConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: aura_intelligenceConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: aura_intelligenceConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: aura_intelligenceConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
