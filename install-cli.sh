...
# Aura Intelligence CLI Installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://auraintelligence.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]

PREFIX="${AURA_PREFIX:-${HOME}/.aura}"
AURA_VERSION="${AURA_VERSION:-latest}"
NODE_VERSION="${AURA_NODE_VERSION:-22.22.0}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.aura)
  ...
  --onboard                           Run "aura_intelligence onboard" after install
...
install_aura() {
  ...
  if [[ "${requested}" == "latest" ]]; then
    if ! SHARP_IGNORE_GLOBAL_LIBVIPS="$SHARP_IGNORE_GLOBAL_LIBVIPS" "$(npm_bin)" install -g --prefix "$PREFIX" "${npm_args[@]}" "aura_intelligence@latest"; then
      ...
      SHARP_IGNORE_GLOBAL_LIBVIPS="$SHARP_IGNORE_GLOBAL_LIBVIPS" "$(npm_bin)" install -g --prefix "$PREFIX" "${npm_args[@]}" "aura_intelligence@next"
      requested="next"
    fi
  else
    SHARP_IGNORE_GLOBAL_LIBVIPS="$SHARP_IGNORE_GLOBAL_LIBVIPS" "$(npm_bin)" install -g --prefix "$PREFIX" "${npm_args[@]}" "aura_intelligence@${requested}"
  fi

  rm -f "${PREFIX}/bin/aura_intelligence"
  cat > "${PREFIX}/bin/aura_intelligence" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "${PREFIX}/lib/node_modules/aura_intelligence/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/aura_intelligence"
...
resolve_aura_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/aura_intelligence" ]]; then
    version="$("${PREFIX}/bin/aura_intelligence" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
main() {
  ...
  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/aura_intelligence" onboard
  fi
}
