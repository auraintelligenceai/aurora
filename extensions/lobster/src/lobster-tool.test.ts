...
import type { aura_intelligencePluginApi, aura_intelligencePluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "aura_intelligence-lobster-plugin-") {
...
function fakeApi(): aura_intelligencePluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<aura_intelligencePluginToolContext> = {}): aura_intelligencePluginToolContext {
  return {
    config: {} as any,
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("tolerates noisy stdout before the JSON envelope", async () => {
    ...
    const { binPath } = await writeFakeLobsterScript(
      `const payload = ${JSON.stringify(payload)};\n` +
        ...
        `process.stdout.write(JSON.stringify(payload));\n`,
      "aura_intelligence-lobster-plugin-noisy-",
    );
  ...
  it("rejects invalid JSON from lobster", async () => {
    const { binPath } = await writeFakeLobsterScript(
      ...
      "aura_intelligence-lobster-plugin-bad-",
    );
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: aura_intelligencePluginToolContext) => {
