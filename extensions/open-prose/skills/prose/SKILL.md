...
metadata: {"aura_intelligence":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == aura_intelligence `sessions_spawn`
- **File I/O** == aura_intelligence `read`/`write`
- **Remote fetch** == aura_intelligence `web_fetch` (or `exec` with curl when POST is required)
