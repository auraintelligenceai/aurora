...
export function listGoogleChatAccountIds(cfg: aura_intelligenceConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: aura_intelligenceConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: aura_intelligenceConfig): ResolvedGoogleChatAccount[] {
