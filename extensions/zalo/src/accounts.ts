...
export function listZaloAccountIds(cfg: aura_intelligenceConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultZaloAccountId(cfg: aura_intelligenceConfig): string {
...
function mergeZaloAccountConfig(cfg: aura_intelligenceConfig, accountId: string): ZaloAccountConfig {
...
export function listEnabledZaloAccounts(cfg: aura_intelligenceConfig): ResolvedZaloAccount[] {
