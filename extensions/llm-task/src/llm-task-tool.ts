...
// NOTE: This extension is intended to be bundled with aura_intelligence.
// When running from source (tests/dev), aura_intelligence internals live under src/.
...
export function createLlmTaskTool(api: aura_intelligencePluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via aura.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aura_intelligence-llm-task-"));
