/**
 * Twitch channel plugin for aura_intelligence.
 *
...
import { buildChannelConfigSchema } from "aura/plugin-sdk";
 ...
 * for aura_intelligence. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: aura_intelligenceConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: aura_intelligenceConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: aura_intelligenceConfig): boolean => {
