...
Display HTML content on connected aura_intelligence nodes (Mac app, iOS, Android).
...
```
http://<tailscale-hostname>:18793/__aura_intelligence__/canvas/<file>.html
```
...
In `~/.clawdbot/aura_intelligence.json`:
...
```bash
cat ~/.clawdbot/aura_intelligence.json | jq '.gateway.bind'
```

Then construct the URL:
- **loopback**: `http://127.0.0.1:18793/__aura_intelligence__/canvas/<file>.html`
- **lan/tailnet/auto**: `http://<hostname>:18793/__aura_intelligence__/canvas/<file>.html`
...
**Example:**
```
canvas action:present node:mac-63599bc4-b54d-4392-9048-b97abd58343a target:http://peters-mac-studio-1.sheep-coho.ts.net:18793/__aura_intelligence__/canvas/snake.html
```
...
**Debug steps:**
1. Check server bind: `cat ~/.clawdbot/aura_intelligence.json | jq '.gateway.bind'`
...
Node is offline. Use `aura_intelligence nodes list` to find online nodes.
...
The canvas host serves from `/__aura_intelligence__/canvas/` prefix:

```
http://<host>:18793/__aura_intelligence__/canvas/index.html  → ~/clawd/canvas/index.html
http://<host>:18793/__aura_intelligence__/canvas/games/snake.html → ~/clawd/canvas/games/snake.html
```

The `/__aura_intelligence__/canvas/` prefix is defined by `CANVAS_HOST_PATH` constant.
