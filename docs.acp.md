# aura_intelligence ACP Bridge

This document describes how the aura_intelligence ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`aura_intelligence acp` exposes an ACP agent over stdio and forwards prompts to a running
aura_intelligence Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a aura_intelligence Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `aura_intelligence acp` over stdio.

Example config:

```bash
aura_intelligence config set gateway.remote.url wss://gateway-host:18789
aura_intelligence config set gateway.remote.token <token>
```

Example run:

```bash
aura_intelligence acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
aura_intelligence acp --session agent:main:main
aura_intelligence acp --session agent:design:main
aura_intelligence acp --session agent:qa:bug-123
```
...
{
  "agent_servers": {
    "aura_intelligence ACP": {
      "type": "custom",
      "command": "aura_intelligence",
      "args": ["acp"],
      "env": {}
    }
  }
...
In Zed, open the Agent panel and select “aura_intelligence ACP” to start a thread.

## Execution Model

- ACP client spawns `aura_intelligence acp` and speaks ACP messages over stdio.
...
```bash
aura_intelligence acp --session agent:main:main
aura_intelligence acp --session-label "support inbox"
aura_intelligence acp --reset-session
```
...
`aura_intelligence acp` resolves the Gateway URL and auth from CLI flags or config:
