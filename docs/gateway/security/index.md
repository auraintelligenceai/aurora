...
- Tighten local perms (`~/.aura_intelligence` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... *spicy*. Here’s how to not get pwned.

aura_intelligence is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, aura_intelligence also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.aura_intelligence/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.aura_intelligence/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.aura_intelligence/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.aura_intelligence/credentials/oauth.json`
...
`aura_intelligence security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${AURA_GATEWAY_PASSWORD}
```
...
aura_intelligence stores session transcripts on disk under `~/.aura_intelligence/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.aura_intelligence` (see the audit section below). If you need
...
aura_intelligence can refresh the skills list mid-session:
...
- If you install plugins from npm (`aura_intelligence plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.aura_intelligence/extensions/<pluginId>/` (or `$AURA_STATE_DIR/extensions/<pluginId>/`).
  - aura_intelligence uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
aura_intelligence pairing list <channel>
aura_intelligence pairing approve <channel> <code>
```
...
By default, aura_intelligence routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
aura_intelligence has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.aura_intelligence/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.aura_intelligence or your logs.”
...
4. **Re-run audit**
   - `aura_intelligence security audit --deep` and confirm the report is clean.
...
- `~/.aura_intelligence/aura_intelligence.json`: `600` (user read/write only)
- `~/.aura_intelligence`: `700` (user only)

`aura_intelligence doctor` can warn and offer to tighten these permissions.

### 0.4) Network exposure (bind + port + firewall)

The Gateway multiplexes **WebSocket + HTTP** on a single port:
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `AURA_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_aura_intelligence-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `AURA_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `aura_intelligence doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `AURA_GATEWAY_PASSWORD`).

Rotation checklist (token/password):
1. Generate/set a new secret (`gateway.auth.token` or `AURA_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), aura_intelligence
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. aura_intelligence verifies the identity by resolving the
...
- aura_intelligence will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.aura_intelligence/` (or `$AURA_STATE_DIR/`) may contain secrets or private data:

- `aura_intelligence.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `aura_intelligence status --all` (pasteable, secrets redacted) over raw logs.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `aura_intelligence gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `AURA_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/aura_intelligence/aura_intelligence-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.aura_intelligence/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + aura_intelligence version
