...
- Replaces the legacy `gateway` command. CLI entry point: `aura_intelligence gateway`.
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__aura_intelligence__/canvas/` from `~/aura/canvas`. Disable with `canvasHost.enabled=false` or `CLAWDBOT_SKIP_CANVAS_HOST=1`.
...
```bash
aura_intelligence --dev setup
aura_intelligence --dev gateway --allow-unconfigured
# then target the dev instance:
aura_intelligence --dev status
aura_intelligence --dev health
```
...
```bash
aura_intelligence --profile main gateway install
aura_intelligence --profile rescue gateway install
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `aura_intelligence health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `aura_intelligence`
  - Arguments: `gateway`
  - KeepAlive: true
...
- If other gateway-like services are detected, the CLI warns unless they are aura_intelligence profile services.
  ...
  - Cleanup: `aura_intelligence gateway uninstall` (current service) and `aura_intelligence doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `aura_intelligence gateway install --force` to reinstall (profile/env/path changes).
...
`aura_intelligence gateway install` writes the user unit. `aura_intelligence doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/aura_intelligence-gateway[-<profile>].service`:
```
[Unit]
Description=aura_intelligence Gateway (profile: <profile>, v<version>)
...
```
systemctl --user enable --now aura_intelligence-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/aura_intelligence-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now aura_intelligence-gateway[-<profile>].service
```
...
## CLI helpers
- `aura_intelligence gateway health|status` — request health/status over the Gateway WS.
- `aura_intelligence message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `aura_intelligence agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `aura_intelligence gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `aura_intelligence gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `aura_intelligence gateway` and the legacy TCP control port.
