...
aura_intelligence only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `aura_intelligence doctor`, `aura_intelligence logs`, `aura_intelligence health`, `aura_intelligence status`, `aura_intelligence service`, `aura_intelligence help`).
- Run `aura_intelligence doctor` to see the exact issues.
- Run `aura_intelligence doctor --fix` (or `--yes`) to apply migrations/repairs.
...
```bash
aura_intelligence gateway call config.get --params '{}' # capture payload.hash
aura_intelligence gateway call config.patch --params '{
...
{ "$include": "/etc/aura_intelligence/base.json5" } // absolute
...
aura_intelligence reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, aura_intelligence runs your login shell and imports only the missing expected keys (never overrides).
...
If set, aura_intelligence derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/aura_intelligence/aura_intelligence-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/aura_intelligence/aura_intelligence.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/aura_intelligence/aura_intelligence.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
...
- The legacy single-account Baileys auth dir is migrated by `aura_intelligence doctor` into `whatsapp/default`.
...
- `commands.config: true` enables `/config` (reads/writes `aura_intelligence.json`).
...
aura_intelligence starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
aura_intelligence starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `aura_intelligence plugins install @aura_intelligence/mattermost` (or `./extensions/mattermost` from a git checkout).
...
aura_intelligence starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
aura_intelligence spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, aura_intelligence
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/aura_intelligence" } }
}
```
...
When a file exceeds this limit, aura_intelligence logs a warning and injects a truncated
...
message envelopes). If unset, aura_intelligence uses the host timezone at runtime.
...
`[aura_intelligence]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[aura_intelligence]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[aura_intelligence]"`, aura_intelligence will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, aura_intelligence generates audio
...
aura_intelligence also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, aura_intelligence currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `aura_intelligence configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:aura_intelligence`: all built-in aura_intelligence tools (excludes provider plugins)
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "aura_intelligence-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "aura_intelligence-sandbox-browser:bookworm-slim",
          containerPrefix: "aura_intelligence-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenCode Zen is a multi-model gateway with per-model endpoints. aura_intelligence uses
...
- Shortcut: `aura_intelligence onboard --auth-choice opencode-zen`.
...
Shortcut: `aura_intelligence onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `aura_intelligence onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `aura_intelligence onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `aura_intelligence onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `aura_intelligence onboard --auth-choice minimax-api`.
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, aura_intelligence stays in idle-only mode for backward compatibility.
...
- Note: the control server is started by the running gateway (aura_intelligence.app menubar, or `aura_intelligence gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/aura_intelligence" }
...
- Examples: `"/ui"`, `"/aura_intelligence"`, `"/apps/aura_intelligence"`.
...
- When a connection comes from one of these IPs, aura_intelligence uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- `x-aura_intelligence-token: <token>` **or**
- `?token=<token>`
...
Gmail helper config (used by `aura_intelligence webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "aura_intelligence@gmail.com",
...
Note: when `tailscale.mode` is on, aura_intelligence defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__aura_intelligence/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__aura_intelligence__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
Controls LAN mDNS discovery broadcasts (`_aura_intelligence-gw._tcp`).
...
- Tailscale **split DNS** so clients resolve `aura_intelligence.internal` via that server
