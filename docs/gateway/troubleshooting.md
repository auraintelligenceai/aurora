---
summary: "Quick troubleshooting guide for common aura_intelligence failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When aura_intelligence misbehaves, here's how to fix it.
...
|---|---|---|
| `aura_intelligence status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `aura_intelligence status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `aura_intelligence status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `aura_intelligence gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `aura_intelligence channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  aura_intelligence models auth setup-token --provider anthropic
  ```
...
```bash
aura_intelligence models auth paste-token --provider anthropic
aura_intelligence models status
```
...
- Or set it directly:
  ```bash
  aura_intelligence config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  aura_intelligence config set gateway.mode remote
  aura_intelligence config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `aura_intelligence setup` to create a starter config, then rerun
the gateway.
...
service was installed with Bun or a version-managed Node path, run `aura_intelligence doctor`
...
- then run `aura_intelligence sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `aura_intelligence gateway` and the service.
...
- Fix: run `aura_intelligence doctor` to update it (or `aura_intelligence gateway install --force` for a full rewrite).
...
If you upgraded from older installs, you might still have `~/aura_intelligence` on disk.
...
aura_intelligence intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `aura_intelligence models list` or
  `aura_intelligence models scan` and choose a supported one.
...
tail -f "$(ls -t /tmp/aura_intelligence/aura_intelligence-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
aura_intelligence logs --follow | grep "pairing request"
```
...
aura_intelligence logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/aura_intelligence/aura_intelligence-*.log | head -1)" | tail -20
```

### High Memory Usage

aura_intelligence keeps conversation history in memory.
...
aura_intelligence now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `aura_intelligence doctor` reports every invalid entry.
- `aura_intelligence doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `aura_intelligence logs`, `aura_intelligence health`, `aura_intelligence status`, `aura_intelligence gateway status`, and `aura_intelligence gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/aura_intelligence/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `aura_intelligence models status` (CLI).
...
```bash
aura_intelligence channels login
```

### Build errors on `main` — what’s the standard fix path?

1) `git pull origin main && pnpm install`
2) `aura_intelligence doctor`
...
5) Run `aura_intelligence channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. aura_intelligence scrubs/normalizes tool
schemas in current `main`, but the fix is not in the last release yet (as of
January 13, 2026).

Fix checklist:
1) **Update aura_intelligence**:
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/aura_intelligence/aura_intelligence/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
Ensure the global `aura_intelligence` CLI is installed and matches the app version:
```bash
aura_intelligence --version
npm install -g aura_intelligence@<version>
```
...
aura_intelligence gateway status --deep
...
tail -20 /tmp/aura_intelligence/aura_intelligence-*.log
```
...
1. Check logs first: `/tmp/aura_intelligence/` (default: `aura_intelligence-YYYY-MM-DD.log`, or your configured `logging.file`)
