...
- During heartbeat runs, aura_intelligence treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `aura_intelligence sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, aura_intelligence skips the heartbeat run entirely (no model call).
...
```bash
aura_intelligence system event --text "Check for urgent follow-ups" --mode now
```
