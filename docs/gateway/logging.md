...
aura_intelligence has two log “surfaces”:
...
- The log file path and level can be configured via `~/.clawdbot/aura_intelligence.json`:
  - `logging.file`
  - `logging.level`
...
`aura_intelligence gateway` supports a per-gateway style switch:
...
aura_intelligence gateway --verbose --ws-log compact

# show all WS traffic (full meta)
aura_intelligence gateway --verbose --ws-log full
```
