---
summary: "Chrome extension: let aura_intelligence drive your existing Chrome tab"
read_when:
...
The aura_intelligence Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate aura-managed Chrome profile.
...
aura_intelligence then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the aura_intelligence release (npm package) as static files. There is no separate “build” step.

After upgrading aura_intelligence:
- Re-run `aura_intelligence browser extension install` to refresh the installed files under your aura_intelligence state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

aura_intelligence ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want aura_intelligence to control.
...
- `ON`: attached; aura_intelligence can drive that tab.
...
Debugging: `aura_intelligence sandbox explain`
...
`aura_intelligence browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `aura_intelligence browser extension install` first to copy the extension to a stable location under your aura_intelligence state directory.
...
- **This is not isolated** like the dedicated aura-managed profile.
