---
summary: "Agent tool surface for aura_intelligence (browser, canvas, nodes, message, cron) replacing legacy `aura_intelligence-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `aura_intelligence-*` skills
---

# Tools (aura_intelligence)

aura_intelligence exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `aura_intelligence-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `aura_intelligence.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, aura_intelligence logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`aura_intelligence node run`).
...
Notes:
- Requires a Brave API key (recommended: `aura_intelligence configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated aura browser.
...
- Quick smoke: `aura_intelligence nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `aura_intelligence gateway` restart in-place)
