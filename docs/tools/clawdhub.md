...
By default, the CLI installs skills into `./skills` under your current working directory. If a aura_intelligence workspace is configured, `clawdhub` falls back to that workspace unless you override `--workdir` (or `CLAWDHUB_WORKDIR`). aura_intelligence loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.aura/skills` or bundled skills, workspace skills take precedence.
...
`clawdhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/aura_intelligence/skills` and `~/.aura/skills`). This is designed to find older skill installs without extra flags.
