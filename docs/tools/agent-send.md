---
summary: "Direct `aura_intelligence agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `aura_intelligence agent` (direct agent runs)

`aura_intelligence agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `aura_intelligence message --target`).
...
```bash
aura_intelligence agent --to +15555550123 --message "status update"
aura_intelligence agent --agent ops --message "Summarize logs"
aura_intelligence agent --session-id 1234 --message "Summarize inbox" --thinking medium
