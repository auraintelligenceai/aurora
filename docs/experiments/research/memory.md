...
read_when:
  - Designing workspace memory (~/aura) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep aura_intelligence integration
  - Adding offline recall + reflection (retain/recall/reflect)
---

# Workspace Memory v2 (offline): research notes

Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/aura`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/aura` as canonical human-readable memory.
...
Recommendation: **deep integration in aura_intelligence**, but keep a separable core library.

### Why integrate into aura_intelligence?
- aura_intelligence already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `aura_intelligence memory recall "…" --k 25 --since 30d`
  - `aura_intelligence memory reflect --since 7d`
