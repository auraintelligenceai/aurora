...
read_when:
  - You want to understand `auraintelligence.ai/install.sh`
...
aura_intelligence ships three installer scripts (served from `auraintelligence.ai`):

- `https://auraintelligence.ai/install.sh` — “recommended” installer (global npm install by default; can also install from a GitHub checkout)
- `https://auraintelligence.ai/install-cli.sh` — non-root-friendly CLI installer (installs into a prefix with its own Node)
 - `https://raw.githubusercontent.com/auraintelligenceai/aurora/main/install.ps1` — Windows PowerShell installer (npm by default; optional git install)

To see the current flags/behavior, run:

```bash
curl -fsSL https://auraintelligence.ai/install.sh | bash -s -- --help
```

Windows (PowerShell) help:

```powershell
& ([scriptblock]::Create((iwr -useb https://raw.githubusercontent.com/auraintelligenceai/aurora/main/install.ps1))) -?
```

If the installer completes but `aura_intelligence` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g aura_intelligence@latest`
...
- If upgrading an existing install: runs `aura_intelligence doctor --non-interactive` (best effort).
- For git installs: runs `aura_intelligence doctor --non-interactive` after install/update (best effort).
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://auraintelligence.ai/install.sh | bash
```

### Discoverability / “git install” prompt

If you run the installer while **already inside a aura_intelligence source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:
...
This script installs `aura_intelligence` into a prefix (default: `~/.aura`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.

Help:

```bash
curl -fsSL https://auraintelligence.ai/install-cli.sh | bash -s -- --help
```
...
- Choose install method:
  - `npm` (default): `npm install -g aura_intelligence@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `aura_intelligence doctor --non-interactive` on upgrades and git installs (best effort).
...
```powershell
iwr -useb https://raw.githubusercontent.com/auraintelligenceai/aurora/main/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://raw.githubusercontent.com/auraintelligenceai/aurora/main/install.ps1 | iex -InstallMethod git -GitDir "C:\\aura_intelligence"
```
...
- **"aura_intelligence" is not recognized**: your npm global bin folder is not on PATH. Most systems use
