...
read_when:
  - "You installed aura_intelligence but `aura_intelligence` is “command not found”"
...
aura_intelligence’s runtime baseline is **Node 22+**.

If you can run `npm install -g aura_intelligence@latest` but later see `aura_intelligence: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
