...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in aura_intelligence
---

# Node + tsx "__name is not a function" crash

## Summary
Running aura_intelligence via Node with `tsx` fails at startup with:

```
[aura_intelligence] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `aura_intelligence status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch aura_intelligence.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node aura_intelligence.mjs status` works on Node 25.
