---
summary: "Use Synthetic's Anthropic-compatible API in aura_intelligence"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. aura_intelligence registers it as the
...
```bash
aura_intelligence onboard --auth-choice synthetic-api-key
```
...
Note: aura_intelligence's Anthropic client appends `/v1` to the base URL, so use
