...
aura_intelligence cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
aura_intelligence cron edit <jobId> --agent ops
aura_intelligence cron edit <jobId> --clear-agent
```
```
...
```bash
aura_intelligence cron edit <jobId> \
...
```bash
aura_intelligence system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`aura_intelligence system event`](/cli/system).
