...
- **Session store** (chat history + routing state) under `~/.aura/agents/<agentId>/sessions`.
...
available from `~/.aura/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
If you do nothing, aura_intelligence runs a single agent:
...
```bash
aura_intelligence agents list --bindings
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/aura-alex" },
      { id: "mia", workspace: "~/aura-mia" }
    ]
  },
  bindings: [
...
```js
{
    ...
    list: [
      {
        ...
        name: "Home",
        workspace: "~/aura-home",
        agentDir: "~/.aura/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/aura-work",
        agentDir: "~/.aura/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.aura/credentials/whatsapp/personal
          // authDir: "~/.aura/credentials/whatsapp/personal",
        },
        biz: {
...
```json5
{
  agents: {
    list: [
      { id: "chat", name: "Everyday", workspace: "~/aura-chat", model: "anthropic/claude-sonnet-4-5" },
      { id: "opus", name: "Deep Work", workspace: "~/aura-opus", model: "anthropic/claude-opus-4-5" }
    ]
  },
  bindings: [
