...
aura_intelligence has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
### Layer 2: Gateway + dev agent smoke (what “@aura_intelligence” actually does)
...
aura_intelligence models list --json
```
...
- Gemini API vs Gemini CLI:
  - API: aura_intelligence calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: aura_intelligence shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `aura_intelligence models scan` to find tool+image capable candidates)
