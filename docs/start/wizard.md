...
The onboarding wizard is the **recommended** way to set up aura_intelligence on macOS,
...
`aura_intelligence dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
...
(`web_fetch` works without a key). Easiest path: `aura_intelligence configure --section web`
...
```bash
aura_intelligence agents add <name>
```
...
1) **Existing config detection**
   ...
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `aura_intelligence doctor` before continuing.
...
2) **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.aura/.env` so launchd can read it.
  ...
  - OAuth credentials live in `~/.aura/credentials/oauth.json`; auth profiles live in `~/.aura/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
   - More detail: [/concepts/oauth](/concepts/oauth)

3) **Workspace**
   - Default `~/aura` (configurable).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `aura_intelligence pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `aura_intelligence health`.
   - Tip: `aura_intelligence status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `aura_intelligence agents add <name>` to create a separate agent with its own workspace,
...
Notes:
- Default workspaces follow `~/aura-<agentId>`.
...
```bash
aura_intelligence onboard --non-interactive \
  --mode local \
  --auth-choice apiKey \
...
- Stores it under `~/.aura/tools/signal-cli/<version>/`.
...
`aura_intelligence agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.aura/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.aura/agents/<agentId>/sessions/`.
