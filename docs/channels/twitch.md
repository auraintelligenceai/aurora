...
read_when:
  - Setting up Twitch chat integration for aura_intelligence
---
# Twitch (plugin)

Twitch chat support via IRC connection. aura_intelligence connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
aura_intelligence plugins install @aura_intelligence/twitch
```

Local checkout (when running from a git repo):

```bash
aura_intelligence plugins install ./extensions/twitch
```
