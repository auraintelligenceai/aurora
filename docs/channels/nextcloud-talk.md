...
```bash
aura_intelligence plugins install @aura_intelligence/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
aura_intelligence plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "aura_intelligence" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `aura_intelligence pairing list nextcloud-talk`
  - `aura_intelligence pairing approve nextcloud-talk <CODE>`
