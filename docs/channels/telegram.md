...
- If Telegram rejects the HTML payload, aura_intelligence retries the same message as plain text.

## Commands (native + custom)
aura_intelligence registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
Notes:
- Custom commands are **menu entries only**; aura_intelligence does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `aura_intelligence logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.

## Config writes
By default, Telegram is allowed to write config updates triggered by channel events or `/config set|unset`.

This happens when:
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). aura_intelligence can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. aura_intelligence:
...
Private chats can include `message_thread_id` in some edge cases. aura_intelligence keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
- Approve via:
  ...
  - `aura_intelligence pairing approve telegram <CODE>`
...
2) Run `aura_intelligence logs --follow` and look for `from.id`.
...
aura_intelligence defaults to audio files for backward compatibility.
...
aura_intelligence supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, aura_intelligence handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, aura_intelligence caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.aura/telegram/sticker-cache.json`
...
aura_intelligence uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, aura_intelligence:
...
**Requirements:**
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by aura_intelligence)
...
- Example: `aura_intelligence message send --channel telegram --target 123456789 --message "hi"`.
...
- `aura_intelligence channels status` shows a warning when config expects unmentioned group messages.
- `aura_intelligence channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `aura_intelligence logs --follow` (look for "skipping group message")
...
- Upgrade to a aura_intelligence build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
