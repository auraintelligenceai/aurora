...
3) Set the token for aura_intelligence:
...
## Goals
- Talk to aura_intelligence via Discord DMs or guild channels.
...
3. Configure aura_intelligence with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `aura_intelligence pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running aura_intelligence in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents aura_intelligence needs
...
Discord uses numeric ids everywhere; aura_intelligence config prefers ids.
...
### Troubleshooting
- First: run `aura_intelligence doctor` and `aura_intelligence channels status --probe` (actionable warnings + quick audits).
...
- On startup, aura_intelligence resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror aura_intelligence’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; aura_intelligence enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`aura_intelligence gateway --force`) after confirming no other processes own the Discord session.
