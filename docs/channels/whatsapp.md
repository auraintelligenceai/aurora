...
2) Configure WhatsApp in `~/.aura/aura_intelligence.json`.
...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run aura_intelligence on WhatsApp:

### Dedicated number (recommended)
Use a **separate phone number** for aura_intelligence. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate — install WhatsApp Business and register the aura_intelligence number there.
...
Quick fallback: run aura_intelligence on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[aura_intelligence]`)
...
## Why Not Twilio?
- Early aura_intelligence builds supported Twilio’s WhatsApp Business integration.
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `aura_intelligence pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run aura_intelligence on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will aura_intelligence message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. aura_intelligence only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:
- First DM from a new sender returns a short code (message is not processed).
- Approve with: `aura_intelligence pairing approve whatsapp <code>` (list with `aura_intelligence pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different aura_intelligences on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `aura_intelligence message send --media <mp4> --gif-playback`
    - Gateway: `send` params include `gifPlayback: true`

## Voice notes (PTT audio)
WhatsApp sends audio as **voice notes** (PTT bubble).
- Best results: OGG/Opus. aura_intelligence rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Log file: `/tmp/aura_intelligence/aura_intelligence-YYYY-MM-DD.log` (configurable).
...
- Fix: run `aura_intelligence channels login` on the gateway host and scan the QR (WhatsApp → Settings → Linked Devices).

**Linked but disconnected / reconnect loop**
- Symptom: `channels status` shows `running, disconnected` or warns “Linked but disconnected”.
- Fix: `aura_intelligence doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `aura_intelligence logs --follow`.
