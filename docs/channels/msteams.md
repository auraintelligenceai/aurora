...
```bash
aura_intelligence plugins install @aura_intelligence/msteams
```

Local checkout (when running from a git repo):
```bash
aura_intelligence plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
aura_intelligence will offer the local install path automatically.
...
3) Configure aura_intelligence with those credentials.
...
## Goals
- Talk to aura_intelligence via Teams DMs, group chats, or channels.
...
- On startup, aura_intelligence resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.aura/aura_intelligence.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring aura_intelligence, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `aura_intelligence-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `aura_intelligence plugins install @aura_intelligence/msteams`
   - From a local checkout: `aura_intelligence plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "aura_intelligence in Teams", "full": "aura_intelligence in Teams" },
...
aura_intelligence handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, aura_intelligence only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/aura_intelligenceShared/` folder in the configured SharePoint site's default document library.
...
**CLI:**
```bash
aura_intelligence message send --channel msteams \
...
aura_intelligence message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
aura_intelligence message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
aura_intelligence message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
aura_intelligence message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
