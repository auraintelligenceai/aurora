...
3) Configure aura_intelligence with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for aura_intelligence + `imsg` (Messages DB access).
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. aura_intelligence only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. aura_intelligence can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, aura_intelligence attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - aura_intelligence gateway           │          SCP (attachments)        │ - Messages signed in     │
...
- Approve via:
  - `aura_intelligence pairing list imessage`
  - `aura_intelligence pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, aura_intelligence treats that thread as a “group” for:
