...
Tlon is a decentralized messenger built on Urbit. aura_intelligence connects to your Urbit ship and can
...
```bash
aura_intelligence plugins install @aura_intelligence/tlon
```

Local checkout (when running from a git repo):

```bash
aura_intelligence plugins install ./extensions/tlon
```
...
Use these with `aura_intelligence message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, aura_intelligence replies in-thread.
