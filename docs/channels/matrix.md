...
Matrix is an open, decentralized messaging protocol. aura_intelligence connects as a Matrix **user**
...
```bash
aura_intelligence plugins install @aura_intelligence/matrix
```

Local checkout (when running from a git repo):

```bash
aura_intelligence plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `aura_intelligence plugins install @aura_intelligence/matrix`
   - From a local checkout: `aura_intelligence plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: aura_intelligence calls the same
     login endpoint, stores the access token in `~/.aura/credentials/matrix/credentials.json`,
...
- On first connection, aura_intelligence requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  aura_intelligence logs a warning.
...
`~/.aura/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `aura_intelligence pairing list matrix`
  - `aura_intelligence pairing approve matrix <CODE>`
...
- On startup, aura_intelligence resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
