...
aura_intelligence can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere aura_intelligence can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, aura_intelligence defaults
...
TTS config lives under `messages.tts` in `aura_intelligence.json`.
...
- If `provider` is **unset**, aura_intelligence prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.aura/settings/tts.json`, override with `CLAWDBOT_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, aura_intelligence retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, aura_intelligence:
...
Discord note: `/tts` is a built-in Discord command, so aura_intelligence registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from aura_intelligence
```
