---
summary: "Top-level overview of aura_intelligence, features, and purpose"
read_when:
  - Introducing aura_intelligence to newcomers
---
# aura_intelligence 🦞
...
- **Guided setup (recommended):** [Wizard](/start/wizard) (`aura_intelligence onboard`)
- **Open the dashboard (local Gateway):** http://127.0.0.1:18789/ (or http://localhost:18789/)

If the Gateway is running on the same computer, that link opens the browser Control UI
immediately. If it fails, start the Gateway first: `aura_intelligence gateway`.
...
WhatsApp / Telegram / Discord / iMessage (+ plugins)
  ...
  │                           │    /__aura_intelligence__/canvas/ (Canvas host)
  └───────────┬───────────────┘
              │
              ├─ Pi agent (RPC)
              ├─ CLI (aura_intelligence …)
              ├─ Chat UI (SwiftUI)
              ├─ macOS app (aura_intelligence.app)
              ├─ iOS node via Gateway WS + pairing
              └─ Android node via Gateway WS + pairing
```

Most operations flow through the **Gateway** (`aura_intelligence gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Loopback-first**: Gateway WS defaults to `ws://127.0.0.1:18789`.
  ...
  - For Tailnet access, run `aura_intelligence gateway --bind tailnet --token ...` (token is required for non-loopback binds).
- **Nodes**: connect to the Gateway WebSocket (LAN/tailnet/SSH as needed); legacy TCP bridge is deprecated/removed.
- **Canvas host**: HTTP file server on `canvasHost.port` (default `18793`), serving `/__aura_intelligence__/canvas/` for node WebViews; see [Gateway configuration](/gateway/configuration) (`canvasHost`).
...
Switching between npm and git installs later is easy: install the other flavor and run `aura_intelligence doctor` to update the gateway service entrypoint.
...
If you don’t have a global install yet, run the onboarding step via `pnpm aura_intelligence ...` from the repo.
...
```bash
aura_intelligence message send --target +15555550123 --message "Hello from aura_intelligence"
```

## Configuration (optional)

Config lives at `~/.clawdbot/aura_intelligence.json`.
...
**aura_intelligence = CLAW + TARDIS** — because every space lobster needs a time-and-space machine.
