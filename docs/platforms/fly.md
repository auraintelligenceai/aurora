---
title: Fly.io
description: Deploy aura_intelligence on Fly.io
---

# Fly.io Deployment

**Goal:** aura_intelligence Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-aura_intelligence

# Create a persistent volume (1GB is usually enough)
fly volumes create aura_intelligence_data --size 1 --region iad
```
...
```toml
app = "my-aura_intelligence"  # Your app name
primary_region = "iad"
...
```bash
mkdir -p /data
cat > /data/aura_intelligence.json << 'EOF'
{
  "agents": {
    "defaults": {
      "model": {
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/aura_intelligence.json` should be read on restart.

Verify the config exists:
```bash
fly ssh console --command "cat /data/aura_intelligence.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/aura_intelligence.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/aura_intelligence.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:
```bash
fly ssh console --command "rm /data/aura_intelligence.json"
```
...
```bash
# List current IPs
fly ips list -a my-aura_intelligence

# Release public IPs
fly ips release <public-ipv4> -a my-aura_intelligence
fly ips release <public-ipv6> -a my-aura_intelligence
...
fly ips allocate-v6 --private -a my-aura_intelligence
```
...
fly proxy 3000:3000 -a my-aura_intelligence
...
**Option 3: SSH only**
```bash
fly ssh console -a my-aura_intelligence
```
