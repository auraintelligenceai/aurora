---
summary: "aura_intelligence on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up aura_intelligence on a Raspberry Pi
  - Running aura_intelligence on ARM devices
  - Building a cheap always-on personal AI
---

# aura_intelligence on Raspberry Pi

## Goal

Run a persistent, always-on aura_intelligence Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u aura_intelligence -f
```
...
# Update config
aura_intelligence config set gateway.bind tailnet
...
Most aura_intelligence features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled aura_intelligence

# Enable if not
sudo systemctl enable aura_intelligence

# Start on boot
sudo systemctl start aura_intelligence
```
...
```bash
# Check logs
journalctl -u aura_intelligence --no-pager -n 100

# Common fix: rebuild
cd ~/aura_intelligence  # if using hackable install
npm run build
sudo systemctl restart aura_intelligence
```
