...
read_when:
  - Installing aura_intelligence on Windows
  - Looking for Windows companion app status
---
# Windows (WSL2)

aura_intelligence on Windows is recommended **via WSL2** (Ubuntu recommended). The
...
- Remote nodes must point at a **reachable** Gateway URL (not `127.0.0.1`); use
  `aura_intelligence status --all` to confirm.
...
### 3) Install aura_intelligence (inside WSL)
