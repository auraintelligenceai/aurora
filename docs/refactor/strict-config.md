...
- Plugin manifests (`aura_intelligence.plugin.json`) are required for all plugins.

## Plugin schema enforcement
- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1) Resolve plugin manifest + schema (`aura_intelligence.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `aura_intelligence doctor --fix`.
...
- `aura_intelligence help`
- `aura_intelligence status`
- `aura_intelligence gateway status`

Everything else must hard-fail with: “Config invalid. Run `aura_intelligence doctor --fix`.”
