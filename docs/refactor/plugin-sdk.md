...
Delivery:
- Publish as `@aura/plugin-sdk` (or export from core under `aura/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `aura_intelligencePluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: aura_intelligenceConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: aura_intelligenceConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: aura_intelligenceConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: aura_intelligenceConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: aura_intelligenceConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: aura_intelligenceConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@aura/plugin-sdk`.
- Add `api.runtime` to `aura_intelligencePluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `aura_intelligenceRuntime: ">=2026.2.0"`).
