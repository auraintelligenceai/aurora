...
Deploy aura_intelligence on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/aura_intelligence/aura_intelligence" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run aura_intelligence:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: AURA_STATE_DIR
        value: /data/.aura
      - key: AURA_WORKSPACE_DIR
        value: /data/workspace
      - key: AURA_GATEWAY_TOKEN
        generateValue: true  # auto-generates a secure token
    disk:
      name: aura_intelligence-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/aura_intelligence`.
...
If you use the original aura_intelligence repository, Render will not auto-deploy your aura_intelligence. To update it, run a manual Blueprint sync from the dashboard.
...
For aura_intelligence, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any aura_intelligence host.
