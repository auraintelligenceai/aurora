...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when aura_intelligence would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When aura_intelligence would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/aura-alfred/
...
Workspace: /Users/pascal/aura-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.aura/logs/gateway.log | grep broadcast
```
...
```typescript
interface aura_intelligenceConfig {
  broadcast?: {
