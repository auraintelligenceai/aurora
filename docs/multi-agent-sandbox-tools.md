...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `aura_intelligence sandbox explain`.
...
Legacy `agent.*` configs are migrated by `aura_intelligence doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=aura_intelligence.sandbox=1"
   ```
