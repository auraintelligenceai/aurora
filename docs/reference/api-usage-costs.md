...
aura_intelligence features that can generate provider usage or paid API calls.
...
- `aura_intelligence status --usage` and `aura_intelligence channels list` show provider **usage windows**
...
aura_intelligence can pick up credentials from:
...
- `aura_intelligence models status --json`
...
### 8) Model scan / probe
`aura_intelligence models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
