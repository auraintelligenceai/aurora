...
This document explains how aura_intelligence manages sessions end-to-end:
...
aura_intelligence is designed around a single **Gateway process** that owns session state.
...
aura_intelligence persists sessions in two layers:
...
aura_intelligence intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (aura_intelligence consumes the events, but Pi decides when to compact).
...
aura_intelligence also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, aura_intelligence bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, aura_intelligence leaves it alone.
...
- `aura_intelligence status` (CLI)
- `aura_intelligence sessions` / `sessions --json`
...
aura_intelligence supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- aura_intelligence strips/suppresses this in the delivery layer.

As of `2026.1.10`, aura_intelligence also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
aura_intelligence uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but aura_intelligence’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `aura_intelligence status`.
