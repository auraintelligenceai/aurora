...
import type { aura_intelligenceConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("aura_intelligence-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("aura_intelligence-bin-sherpa-");
      const modelDir = await makeTempDir("aura_intelligence-sherpa-model-");
      ...
      const cfg: aura_intelligenceConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("aura_intelligence-bin-whispercpp-");
      const modelDir = await makeTempDir("aura_intelligence-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("aura_intelligence-bin-gemini-");
      ...
      const cfg: aura_intelligenceConfig = { tools: { media: { image: {} } } };
