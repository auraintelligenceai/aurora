...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `aura_intelligence-e2e-${name}-`));
  const configDir = path.join(homeDir, ".aura");
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        AURA_GATEWAY_TOKEN: inst.gatewayToken,
        AURA_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AURA_GATEWAY_PORT: String(gwA.port),
          AURA_GATEWAY_TOKEN: gwA.gatewayToken,
          AURA_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          AURA_GATEWAY_PORT: String(gwB.port),
          AURA_GATEWAY_TOKEN: gwB.gatewayToken,
          AURA_GATEWAY_PASSWORD: "",
        }),
