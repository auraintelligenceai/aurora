...
mkdir -p "${CLAWDBOT_CONFIG_DIR:-$HOME/.aura}"
mkdir -p "${CLAWDBOT_WORKSPACE_DIR:-$HOME/aura}"

export CLAWDBOT_CONFIG_DIR="${CLAWDBOT_CONFIG_DIR:-$HOME/.aura}"
export CLAWDBOT_WORKSPACE_DIR="${CLAWDBOT_WORKSPACE_DIR:-$HOME/aura}"
...
echo ""
docker compose "${COMPOSE_ARGS[@]}" run --rm aura_intelligence-cli onboard --no-install-daemon

echo ""
echo "==> Provider setup (optional)"
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm aura_intelligence-cli providers login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm aura_intelligence-cli providers add --provider telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm aura_intelligence-cli providers add --provider discord --token <token>"
echo "Docs: https://docs.auraintelligence.ai/providers"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d aura_intelligence-gateway
